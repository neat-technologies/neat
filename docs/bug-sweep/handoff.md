...
| 789 | crit | MCP tools never driven vs a live daemon — query-correctness gap | Fixed by live MCP capture assertions; STATIC and OBSERVED expectations are separated correctly. | `e2e/capture/mcp-assertions.ts`, `run.sh`, capture CI. | **MERGED** | #857 |
| 745 | crit | web graph pane stuck 'loading'; project=null → 502 | **ALREADY FIXED on main / reported against stale 0.4.27 `vercel-testing`** — current `resolveProfile` auto-selects the first running+reachable profile and graph consumers gate while project is null. Focused resolution/cold-load tests pass 16/16, including the exact single-profile case. | none — close the issue | **VERIFIED-FIXED → close #745** | — |
| 838 | crit | long file-grained node ids → misleading route-404 over REST | **ALREADY FIXED on main** — `531a27f`/#839 raises Fastify `routerOptions.maxParamLength` to 1024 and adds handler-level long-id coverage. Verified ancestor of HEAD; focused tests 4/4. | none — close the issue | **VERIFIED-FIXED → close #838** | #839 |
...
| 789 | crit | (see above) MCP tools not driven vs a live daemon | Fixed by live MCP capture assertions; STATIC query expectation corrected to avoid requiring an OBSERVED-only target. | `e2e/capture/mcp-assertions.ts`, `run.sh`, capture CI. | **MERGED** | #857 |
| 830 | high | ESM/TS otel-init doesn't survive missing `@opentelemetry` deps | **CONFIRMED REAL** — both templates used hoisted static imports, so missing packages fail during ESM linking before any runtime guard can catch the error. | Guard top-level dynamic imports with `try/catch`; retain top-level await ordering; add an ESM missing-deps boot regression. | **MERGED** | #860 |
| 824 | med | orphan `/api/stale-events` route + #804 dead-code sweep | **REAL** — production search confirms no consumer under `packages/web/app`; the only apparent consumer was an inaccurate `StatusBar` stub in `single-project-resolution.test.tsx`, while the real component polls `/api/health`. | Removed the orphan proxy and made the test stub mirror the real health request. | **MERGED** | #861 |
...
- **2026-07-21** — **#824 confirmed real and merged (#861).** Removed the orphan web `/api/stale-events` proxy; corrected the single-project-resolution `StatusBar` stub to mirror production's `/api/health` request. The core `/stale-events` API remains supported for CLI/MCP consumers. CI green.
- **2026-07-21** — **PR #861 CI stabilization.** Fixed the unrelated `logs-store` inclusive-boundary test flake exposed by CI: entry timestamps and the `since` cutoff now derive from one captured base time, so wall-clock drift cannot exclude the boundary entry.
...
No implementation work remains. Because every fix PR used `Refs` rather than `Closes`, the user should manually close the issues as appropriate:
