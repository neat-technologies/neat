...
| 824 | med | orphan `/api/stale-events` route + #804 dead-code sweep | **REAL** — production search confirms no consumer under `packages/web/app`; the only apparent consumer was an inaccurate `StatusBar` stub in `single-project-resolution.test.tsx`, while the real component polls `/api/health`. | Remove the orphan proxy and make the test stub mirror the real health request. | **PR OPEN** | pending |
...
- **2026-07-21** — **#824 confirmed real and fixed (PR pending).** Removed the orphan web `/api/stale-events` proxy; corrected the single-project-resolution `StatusBar` stub to mirror production's `/api/health` request. The core `/stale-events` API remains supported for CLI/MCP consumers.
