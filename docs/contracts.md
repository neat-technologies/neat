...
| 11 | `getBlastRadius` | [`contracts/get-blast-radius.md`](./contracts/get-blast-radius.md) | BFS inbound (the origin's dependents) default depth 10, distance positive, per-node path + cascaded confidence, schema-validated (ADR-110 inbound-dependents direction; ADR-038) | ✅ landed (v0.2.3 opens) |
...
| 13 | REST API | [`contracts/rest-api.md`](./contracts/rest-api.md) | Root mount is the model (ADR-096); `/projects/:project` still ships but is legacy per ADR-229, which supersedes ADR-026's dual-mount clause. Locked endpoint set, JSON errors, Zod-validated bodies (ADR-040) | ✅ landed (v0.2.4 opens) |
...
| 25 | Publish system | [`contracts/publish-system.md`](./contracts/publish-system.md) | Bin-wrapper subpath validity against dependency `exports`, version lockstep across six packages, tarball smoke-test gate with per-dep visibility wait + web-artifact presence + post-`neatd` liveness + fixture registry shape, dependency order, npm immutability acknowledged; MCP Registry manifest (`server.json` name↔`mcpName` + version lockstep, additive OIDC publish) (ADR-052 + ADR-064 + ADR-153) | ✅ landed (post-0.2.6 broken-publish; gate hardened post-0.3.0; registry manifest v0.6.x) |
...
| 43 | Project daemon | [`contracts/project-daemon.md`](./contracts/project-daemon.md) | One daemon per project; each owns its graph/ports/OTLP/dashboard/MCP and self-describes in `neat-out/daemon.json`; ports allocated once and reused; project root carries one project (no dual-mount, no `default`); machine-wide running-list is append-only and lock-free; matches the hosted per-project shape (ADR-096) | ✅ mostly landed — daemon.json self-description, once-allocated ports, crash reconciliation (ADR-112) shipped; ADR-229 rules ADR-096 the model and marks the `/projects/:project` mount legacy, with its removal a change of its own |
| 44 | Web shell IA | [`contracts/web-shell.md`](./contracts/web-shell.md) | Multi-page SaaS shell whose spine is the fused graph; sidebar page-nav, topbar per-daemon **profile** switcher (ADR-101: profiles discovered from `~/.neat/daemons/*.json` / the platform list, API base = the selected profile's `endpoint` at the daemon root, no `/projects/:name`, no `default`; status = `running\|stopped` liveness, reachability confirmed before auto-select) + ⌘K palette; canvas is one page among list/table views; divergence demoted to a peer query, blast-radius/dependencies/root-cause are node-scoped actions not pages; Policies violation view wires live, the enforcement layer renders as explicit `preview` per #26 until the governance kernel ships (ADR-097 + ADR-101) | ✅ landed — multi-page shell shipped (AppShell / TopBar / ⌘K palette / pages) |
| 45 | Live canvas layout | [`contracts/canvas-layout.md`](./contracts/canvas-layout.md) | Deterministic ELK `layered` for structure (load + explicit re-tidy only); incremental in-place for the SSE stream — pin existing positions, place only the new node near its neighbor, batch ~750ms, pulse-in, never auto-reflow; the two-mode observed-overlay (Mode A idle / Mode B didn't-engage with diagnosis + fix) framed as fusion/completion, incomplete → completing → complete; the overlay is escapable (always-visible close, backdrop-dismiss, per-project dismissal, capped height — never a full-canvas trap) and Mode B needs a real audit signal (`/api/instrumentation` `engaged?`), falling back to Mode A absent one (ADR-098 + ADR-101) | ✅ landed — GraphCanvas + ObservedOverlay (elkjs / cytoscape-elk) |
| 46 | Design system | [`contracts/design-system.md`](./contracts/design-system.md) | Adopt the vendored jedorini component system (shadcn/Base UI, DM Mono, hard corners `--radius: 0`, monochrome + the OBSERVED green `#5fcf9e`); `packages/web` migrates Tailwind v3→v4 (full-dashboard migration + visual-regression pass) and stays on `@base-ui/react` (`^1.6.0`, shared with the vendored jedorini source) with a 1.x compat pass; React 18 / Next 14 stay (ADR-099 + ADR-101) | ✅ landed — Tailwind v4 + @base-ui/react + jedorini tokens |
| 47 | Client profiles | [`contracts/client-profiles.md`](./contracts/client-profiles.md) | One profile `{ endpoint, authToken? }` is how every client (GUI, CLI, MCP) reaches a daemon; talk to the daemon at its root (ADR-096); precedence explicit-profile → `NEAT_CORE_URL` → local `daemon.json` discovery → loopback; reads route to any endpoint including hosted, local mutations stay local; the profile source is the only local↔hosted swap point (ADR-102) | 🟡 partial — seam primitives shipped; named-profile store (`~/.neat/profiles.json`, `--profile`) deferred, likely with hosted |
| 48 | Hosted storage | [`contracts/hosted-storage.md`](./contracts/hosted-storage.md) | Hosted NEAT keeps the graph, embeddings, and bounded traversal in one Postgres — relational nodes/edges, a `pgvector` column for fuzzy retrieval, recursive CTEs for blast-radius/root-cause/dependencies/divergence at the local depth caps; vectors retrieve, never gate; no dedicated graph DB, search engine, or fork until CTE traversal strains; local (graphology + in-process embeddings) unchanged, the backend is the local↔hosted seam (ADR-103) | 🟡 governs neat-infra — Postgres/pgvector store built there; core keeps only the `persist.ts` local↔hosted seam; split pending |
| 49 | Contract enforcement | [`contracts/contract-enforcement.md`](./contracts/contract-enforcement.md) | How contracts are enforced — four pillars matched to clause type (lint/CI · the breaker · the NEAT-on-NEAT policy overlay · review); every contract carries an `enforcement:` tag; new contracts ship with a tag + ≥1 active pillar or an explicit review-only; NEAT-on-NEAT (contracts as graph-pattern policies over the self-graph, gated by the kernel) is the destination (ADR-104) | ✅ landed — lint pillar + `enforcement:` tags shipped (#571); NEAT-on-NEAT policy pillar future |
| 50 | Policy overlay | [`contracts/policy-overlay.md`](./contracts/policy-overlay.md) | L1 = constraints over the graph; a policy is a stored graph query in L2's vocabulary, typed by a schema, with a vector index for fuzzy reach; the graph gates (deterministic subgraph match), vectors only resolve bindings upstream and never enforce; retrieval two-mode (fuzzy recall + graph worst-case), graph-only at the gate; policies reach agents via blast-radius injection; local = graphology, hosted = pgvector + CTEs (ADR-105; frames ADR-093/094/095) | 🟡 partial — matchPolicyToNode + divergence eval as working proof; hard subgraph gate unscheduled |
| 51 | Autonomous remediation | [`contracts/archive/autonomous-remediation.md`](./contracts/archive/autonomous-remediation.md) | The runner — propose→assess→gate→graduate on the kernel: stage a change as FRONTIER, check blast radius + the policy gate against `real ∪ delta`, block refuses / pass graduates / unconfirmed culls; the agent proposes, the deterministic gate decides; local ("run agents in your code") + hosted ("remediation by us"); adds no new enforcement primitive (ADR-106) | ⚪ retired (ADR-217) — never built; kernel gate never shipped, launch used the soft guardrail (#53); archived, revisit with Sniper |
| 52 | Hosted platform | [`contracts/hosted-platform.md`](./contracts/hosted-platform.md) | The managed suite (Supabase-shape) — an outer layer wrapping the tenant-agnostic core; the only local↔hosted swap point is the profile source (platform list) + bearer; Postgres+pgvector storage; the remediation runner runs hosted as its execution venue; wraps, never forks (ADR-107) | 🟡 governs neat-infra — managed suite built there; core stays tenant-agnostic (daemon.ts negative seam only); split pending |
| 53 | Policies soft guardrail | [`contracts/policies-soft-guardrail.md`](./contracts/policies-soft-guardrail.md) | The launch form of policies — context injection, not a gate: the reachable policies (blast-radius injection, ADR-105 §5) are surfaced into agent context via `check_policies` + a memory hook so the agent is aware; it informs, never blocks; the hard gate is the kernel (ADR-093), post-launch; authoring stays plain `policy.json`; graduates to the gate when the kernel lands (ADR-108) | ✅ landed — `check_policies` applicableTo (informs, never blocks); extended with field-guard (#947) |
| 54 | Connectors | [`contracts/connectors.md`](./contracts/connectors.md) | The connectors plane — a second OBSERVED ingestion path (pull) alongside OTLP (push); one provider interface (`packages/core/src/connectors/**`), passive/ambient only, never forces provider traffic; two credential profiles (local — dev's own creds, on-demand poll; hosted — brokered least-privilege creds, continuous metered poll) sharing one pull/map/fuse implementation per provider; fusion reconciles onto the same file-grain call site OTLP ingest targets, honest service-level fallback otherwise; every provider's outbound call routes through a shared junction layer (timeout, retry, per-account rate limiting); Supabase, Railway, Firebase, Cloudflare Workers/Pages, Neon, and Cloud Run are built pull providers; Vercel joins as a provider-push/OTLP **Drains** connector (ADR-146), a second connector shape (ADR-124 + ADR-127 + ADR-128 + ADR-129 + ADR-131 + ADR-146 + ADR-156 + ADR-165) | ✅ six pull providers + junction + file-grain shipped and hardened (#803, #815, #841); Vercel-via-Drains (ADR-146); hosted-profile credential brokering pending |
| 55 | Connector configuration | [`contracts/connector-config.md`](./contracts/connector-config.md) | How a built connector actually turns on — `~/.neat/connectors.json` (machine-level, atomic, `0600`, sibling to `projects.json`) holds one entry per connector (`provider`, optional `project`, `credential`, `options`); a `credential` is an env-var reference by default (`"$SUPABASE_KEY"`), resolved only at daemon-read time so no secret sits at rest, plaintext the explicit opt-in fallback, never the snapshot; `neat connector add/list/remove/test` is a new top-level command family, not an eleventh locked query verb, with `add` validating the credential against the provider's auth path by default (`--skip-validate` escape, an unset env-ref failing as `"$VAR is unset"` not a validation failure); a data-driven provider dispatch table (`connectors/registry.ts` — factory + validator + field schema) is the one place a new provider registers; the daemon reads the config at slot bootstrap, resolves env-refs, and populates `opts.connectors` for the already-built `startConnectorPollLoop` (ADR-130) | ✅ shipped — `~/.neat/connectors.json` + `neat connector add/list/remove/test` + `connectors/registry.ts` |
| 56 | Logs | [`contracts/logs.md`](./contracts/logs.md) | One unified logs surface — native OTLP logs signal (structured-logger output via a new `/v1/logs` receiver, no bare `console.log` capture) plus connector-sourced OCloud logs (Supabase/Railway/Firebase/Cloudflare/Vercel), merged into one bounded per-project, per-source ring buffer; one REST endpoint (`GET /logs`) every consumer reads through; MCP's `get_logs` and the CLI's `neat logs` filter by the same `source`/`service`/`limit`/`since` params the frontend's Logs page uses (ADR-132) | 🟡 backend shipped (`/logs` receiver + per-project ring buffer, `logs-store.ts`); `neat logs` CLI + `get_logs` MCP surfaces still pending |
| 57 | Action ↔ NEAT-host seam | [`contracts/action-hosted-seam.md`](./contracts/action-hosted-seam.md) | The neat-action reads its verdict from any NEAT host via `GET /graph/divergences` + `GET /graph/observed-dependencies/:nodeId`, `Authorization: Bearer` when a token is set, degrading to the static tier on error; one client serves neat-local / self-hosted / hosted; the hosted plane's account-linking + repo→project + multi-tenant scoping are the Action's requirement here, implemented in neat-infra (ADR-187/188) | 🟡 proposed — Action side landed; hosted-plane half reconciles with neat-infra |
...
- **FRONTIER** — the surface tense (ADR-094 / ADR-226): a real, persisted edge for what sits *outside* the settled graph, facing two ways — the **unobservable present** (a hop NEAT reached toward but cannot see: a hung service, an external API) and the **not-yet-real future** (a proposed deploy, a would-be edge). Excluded from `PROV_RANK` and settled traversal (the edge-level twin of the FrontierNode node-gating). Graduates to OBSERVED by **promotion** (present face, sensor-written, ADR-226) or through the **gate** (future face, kernel-written, ADR-093), else is culled. Distinct from the *FrontierNode* node-type (an unresolved span peer, ADR-023/068) — same word, different axis. Detailed in [`provenance.md`](./contracts/provenance.md).
