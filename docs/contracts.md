...
| 54 | Connectors | [`contracts/connectors.md`](./contracts/connectors.md) | The connectors plane — a second OBSERVED ingestion path (pull) alongside OTLP (push); one provider interface (`packages/core/src/connectors/**`), passive/ambient only, never forces provider traffic; two credential profiles (local — dev's own creds, on-demand poll; hosted — brokered least-privilege creds, continuous metered poll) sharing one pull/map/fuse implementation per provider; fusion reconciles onto the same file-grain call site OTLP ingest targets, honest service-level fallback otherwise; Supabase, Railway, Firebase, and Cloudflare Workers/Pages are built providers, Vercel ships as an installer path instead (ADR-124 + ADR-127 + ADR-128 + ADR-129) | ✅ scaffold + four providers landed |
