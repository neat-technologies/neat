...
| 54 | Connectors | [`contracts/connectors.md`](./contracts/connectors.md) | The connectors plane — a second OBSERVED ingestion path (pull) alongside OTLP (push); one provider interface (`packages/core/src/connectors/**`), passive/ambient only, never forces provider traffic; two credential profiles (local — dev's own creds, on-demand poll; hosted — brokered least-privilege creds, continuous metered poll) sharing one pull/map/fuse implementation per provider; fusion reconciles onto the same file-grain call site OTLP ingest targets, honest service-level fallback otherwise; every provider's outbound call routes through a shared junction layer (timeout, retry, per-account rate limiting); Supabase, Railway, Firebase, and Cloudflare Workers/Pages are built pull providers; Vercel joins as a provider-push/OTLP **Drains** connector (ADR-146), a second connector shape (ADR-124 + ADR-127 + ADR-128 + ADR-129 + ADR-131 + ADR-146) | ✅ four pull providers + junction + file-grain shipped and hardened (#803, #815, #841); Vercel-via-Drains (ADR-146); hosted-profile credential brokering pending |
