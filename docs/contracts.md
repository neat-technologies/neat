...
| 55 | Connector configuration | [`contracts/connector-config.md`](./contracts/connector-config.md) | How a built connector actually turns on — `~/.neat/connectors.json` (machine-level, atomic, `0600`, sibling to `projects.json`) holds per-connector credentials/config; `neat connector add/list/remove/enable/disable` is a new top-level command family, not an eleventh locked query verb; a provider dispatch table (`connectors/registry.ts`) is the one place a new provider registers; the daemon reads the config on the existing registry-reload path (ADR-130) | 🟡 contract-only (implementation pending) |
