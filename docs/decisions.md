...
**Status:** Accepted. Refs #1045. Builds on ADR-141 (declared DB host fusion), ADR-152 (table recovered from `db.statement`), ADR-157 §3 (declared columns), ADR-180 (GORM data axis, the closest sibling), ADR-196 (C#/.NET discovery + symbol grain). Amends [`static-extraction.md`](contracts/static-extraction.md).
**Contract:** [`static-extraction.md`](contracts/static-extraction.md).
...
- Pinned by unit + e2e tests (`extract-csharp-datastore.test.ts`, 17 cases): `[Table]` / `ToTable` / verbatim / qualified-name reads, the `nameof` and comment-body negatives, the gate declining a stray `[Table]` with no EF signal; the Npgsql literal and env-driven (with `${VAR}` interpolation) connection strings, the StackExchange.Redis literal and the cross-file (`VALKEY_ADDR` in one file, `Connect` in another) case, and the placeholder fallback; and an end-to-end `extractFromDirectory` over an otel-demo-shaped tree asserting the three concrete ids, engines, and the file-grained CONNECTS_TO / CALLS edges with `evidence.file`.
