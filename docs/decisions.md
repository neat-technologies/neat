...
## ADR-223 — Refreshable connector credentials mint and refresh a short-lived cloud token, GCP service-account first

**Status:** Accepted. Refs #1121. Amends [`connector-config.md`](contracts/connector-config.md) (adds §9, the refreshable credential form) and touches [`connectors.md`](contracts/connectors.md) §3's credential-profile discipline. The credential-resolution half of the connector-config seam; the delegated authorization-code onboarding UX (#793) is the hosted control plane's concern (neat-infra) and stays deferred.

### Context

The GCP connectors — Cloud Run (ADR-165), Firebase (ADR-128), the GCP Load Balancer (ADR-218) — consume a short-lived OAuth access token (Google issues a ~1h token) and correctly hold that minting it is not the connector's job (`poll()` consumes an already-minted token, connectors.md §3). But the resolution seam beneath them mints nothing: a `credential` env-ref resolves to a fixed value **once**, at slot bootstrap (`buildRegistration` → `resolveEntryCredentials`), and the poll loop reuses that value every tick for the daemon's whole life. A static `$SUPABASE_KEY` is a long-lived secret, so resolving it once is correct. A GCP access token is not — a daemon polling for longer than an hour holds a dead token, and every GCP poll fails until a restart re-resolves the env-ref. This is the wall a hosted GCP deployment hits on day one, and the same wall AWS STS (assumed-role, ~1h) and Azure client-credentials tokens will hit.

The fix belongs at the resolution seam, not in any connector: the credential a poll receives must be re-derived on a cadence, minting and caching a fresh token behind its own expiry. The JWT-bearer service-account flow was confirmed against Google's own docs rather than recalled (ADR-150/152 discipline; `developers.google.com/identity/protocols/oauth2/service-account`): a `POST https://oauth2.googleapis.com/token`, `grant_type=urn:ietf:params:oauth:grant-type:jwt-bearer`, `assertion=<signed JWT>` (form-urlencoded); the JWT header is `{ alg: "RS256", typ: "JWT" }`; the claim set is `iss` (the service-account `client_email`), `scope` (space-delimited), `aud` (the token endpoint exactly), `iat`, and `exp` (at most `iat + 3600`); the response is `{ access_token, token_type: "Bearer", expires_in }`. Signing is RS256 over the service-account private key — Node's built-in `crypto` does it, so no dependency joins the tree, and the exchange itself routes through the shared junction like every other outbound connector call (ADR-131).

### Decision

A connector `credential` gains a third form beyond the env-ref string and the multi-field map: a **refreshable credential**, an object carrying a reserved `kind` string that names a credential-source factory (connector-config.md §9). Where a static credential resolves to a value, a refreshable one resolves to a `CredentialSource` — an async `() => Promise<Record<string, unknown>>` (`connectors/types.ts`) the poll loop calls each tick to obtain that tick's credentials, minting and caching a fresh token behind its own expiry logic.

Nothing about `poll()` changes. The connector still receives an opaque, already-minted `credentials` record and performs no auth handshake of its own — the refresh is entirely at the resolution seam, invisible below `poll()`, so a token-refreshing connector and a static-credential one are indistinguishable to every provider. **The GCP connectors are not touched**: the source hands back `{ projectId, accessToken }`, the exact shape they already read.

The seam is three small, additive pieces:

- **`CredentialSource` on the registration + loop** (`connectors/index.ts`). `ConnectorRegistration.credentialSource?` and a matching `ConnectorPollLoopOptions.credentialSource?`; the loop's `tick()` calls it before each poll to build that tick's `credentials`, falling back to the static `ctx.credentials` when absent. A mint that throws fails the tick like any poll error — recorded to the status tracker (ADR-136), `since` held — so a transient token-endpoint outage doesn't skip the gap. Every existing connector leaves `credentialSource` undefined and is byte-for-byte unaffected.
- **The credential-source dispatch** (`connectors/credential-sources.ts`), parallel to the provider dispatch: a `kind` → `{ build, validate }` table. `gcp-service-account` is the first entry. `build` resolves the durable-secret env-ref (`keyJson`, the service-account key JSON — an env-ref by default, exactly like every other credential, so no secret sits at rest, connector-config.md §2) and returns a cached token source over it; `validate` mints one real token, the refreshable analog of a provider's auth probe. Scope defaults to `https://www.googleapis.com/auth/logging.read` (the narrow Cloud Logging read the GCP connectors need), overridable per entry.
- **The GCP token source** (`connectors/gcp-auth.ts`): parse the key, sign the assertion JWT (RS256, Node `crypto`), exchange it through the junction, cache the token with its stated expiry, and re-mint only when a tick arrives within a 60s skew of expiry — so per-tick calls are cheap and a burst of ticks shares one in-flight mint rather than fanning out.

`neat connector add`/`test` validate a refreshable credential by minting a token (proving the key signs and Google accepts it) instead of the provider's static auth probe; an unset key env-ref stays the distinct `unset-env` outcome, kept apart from a malformed or rejected key exactly as the static path keeps it (connector-config.md §4). The daemon-read path (`buildRegistration` → `startConnectorPolling`) threads the source into the loop; a bad refreshable entry is skipped with a reason, never fatal to the slot (connector-config.md §6).

### Consequences

- A long-running or hosted daemon polling GCP holds a live token indefinitely — the staleness that made hosted GCP connectors a restart-to-survive proposition is gone, with no change to any connector's own code.
- The seam is provider-agnostic and kind-extensible: AWS STS assumed-role (external-id for the cross-account hosted case) and Azure client-credentials become new `kind` entries on the same `CredentialSource` contract, not new plumbing. The refresh mechanics — mint, cache, re-mint on skew, one in-flight — are written once here.
- This is the machine-credential half of the credential story. A user delegating their own cloud account by clicking "grant NEAT read access" (#793) produces a refresh token whose storage and rotation is the hosted control plane's concern (neat-infra, team-ownership seam); when it brokers such a token, it injects it as the env-ref the refreshable credential already resolves — the same local↔hosted seam the static env-ref is (connector-config.md §8). This ADR builds to that seam and stops at it.
- The service-account key JSON is a durable secret and the highest-value one a refreshable credential holds. It rides as an env-ref by default (never at rest in `connectors.json`, never in the snapshot, connectors.md §6), the private key is never logged or echoed — only its absence or a parse failure is reported — and a token-endpoint error body is never surfaced, since it can restate the assertion.
- No new dependency and no new query: the JWT is signed with Node's built-in `crypto`, the exchange goes through the existing junction, and the whole change is additive — every static-credential connector's resolution, poll loop, and tests are unchanged.
