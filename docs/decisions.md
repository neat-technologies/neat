...
The compatibility-expansion rubric scores the GCP external HTTP(S) Load Balancer: reach **5/5** (the front door for GKE and Compute Engine workloads, and the surface that has no per-service request log of its own the way Cloud Run does — an LB in front of a GKE Deployment is exactly the case Cloud Run's connector can't see), OBSERVED tractability **4/5** (request logs are auto-emitted with zero app instrumentation, but only ever at route/backend grain — an un-instrumented backend emits no runtime `code.*` call-site stamp, so file precision can come only from a route's own static definition site, never a live stack walk), extraction tractability **5/5** (`extract/routes.ts` already parses the apps the LB fronts), fusion-key clarity **4/5** (the `(method, path)` → `RouteNode` join is the same route-match path Cloud Run, Railway, Render, and Firebase use; the `backend_service_name` → NEAT service step needs a config-time map, resolved once, never guessed), strategic fit **5/5** (the GCP on-ramp for the whole non-serverless compute tier). Total **23/25**, the same as Cloud Run — and it reuses Cloud Run's fetch/auth path wholesale, so the marginal cost is a filter and a label.

Every field below was confirmed against Google's own documentation rather than recalled, per the connectors build discipline (ADR-150/152 — the attribute a convention names is often not the one the API returns):

- **Cloud Logging `entries.list`** — `POST https://logging.googleapis.com/v2/entries:list`, request body `{ resourceNames, filter, orderBy, pageSize, pageToken }`, response `{ entries, nextPageToken }` (`cloud.google.com/logging/docs/reference/v2/rest/v2/entries/list`). The same surface the Cloud Run and Firebase connectors already poll.
- **The LB request-log resource** — the monitored resource type is `http_load_balancer`, carrying labels `backend_service_name`, `forwarding_rule_name`, `target_proxy_name`, `url_map_name`, `zone`, `project_id` (`cloud.google.com/logging/docs/api/v2/resource-list`, cross-checked against the load-balancing logging guide). This connector reads `backend_service_name` — the backend the LB routed to, the fusion twin of Cloud Run's `service_name`.
- **The request-log name** — `projects/<PROJECT_ID>/logs/requests` (`cloud.google.com/load-balancing/docs/https/https-logging-monitoring`). Unlike Cloud Run's `run.googleapis.com%2Frequests`, the LB log id is a plain `requests` with no `/` to URL-encode. Request logging is enabled per backend service; where it is on, entries are emitted automatically with no logging agent and no app change.
- **The `httpRequest` payload** — `requestMethod`, `requestUrl`, `status` (integer), plus `latency`, `remoteIp`, `requestSize`, `responseSize`, `serverIp`, `userAgent`, `referer` (`cloud.google.com/logging/docs/reference/v2/rest/v2/LogEntry`). `requestUrl` on the external LB is typically a full absolute URL (scheme+host+path), so the mapper parses both an absolute URL and a bare path. The load-balancing guide documents that `httpRequest.protocol` is **not** populated for `resource.type = "http_load_balancer"`, so this connector does not read it. The entry's `timestamp` is the request's own event time, distinct from `receiveTimestamp` (ingest time).
- **Least-privilege read** — `entries.list` needs `logging.logEntries.list`, carried by the predefined read-only role `roles/logging.viewer`; a custom role holding only `logging.logEntries.list` is narrower still. This role reads logs only and reaches no customer data — the same finding Cloud Run and Firebase recorded, so both profiles use the same grant with no local/hosted split.

The Cloud Run connector (ADR-165) reads the same Cloud Logging surface, but only a `cloud_run_revision` resource's own request log — it cannot see a request that an LB routed to a GKE or Compute Engine backend, because those backends write no equivalent per-service request log. The external Application Load Balancer's own `http_load_balancer` request log is where that traffic is recorded, and it deserves a connector that reads exactly it.

### Decision

The GCP HTTP(S) Load Balancer joins the pull registry as provider `gcp-lb`, cloning Cloud Run's `{client,index,map,resolve,types}.ts` provider layout. Its credential is `{ projectId, accessToken }`, identical to Cloud Run's: `projectId` is not a secret (a plaintext literal is expected); `accessToken` is a short-lived OAuth token minted from a service-account key or ADC scoped to `roles/logging.viewer`. The connector consumes an already-minted token and performs no auth handshake of its own — the same shape Cloud Run, Firebase, and Railway keep. `docs/connectors/gcp-lb.md` requires revoking every grant beyond logging read.

`poll()` runs `entries.list` once per tick, filtered to the external LB's own request log specifically:
...
ordered `timestamp asc`, paginated to a bounded page count, every call through the shared junction (timeout, retry, per-account rate limit keyed on the GCP project id — the same bucket Cloud Run and Firebase use). Pinning the log name plus the `http_load_balancer` resource type is what scopes this connector to the LB's own request record.

Dedup is a timestamp watermark, not a baseline — a request log is a per-request event, so `since` advances to each tick's start and the filter lower-bounds the next window; a first poll with no watermark backfills a bounded 24h lookback, never an unbounded full-history replay. This is Cloud Run's watermark discipline verbatim.

Mapping is 1:1 — one log entry becomes one `ObservedSignal` carrying `(backend_service_name, method, path)`, `callCount: 1`, `errorCount` set at the 5xx threshold (a bare 4xx is often correct app behavior, the same line ingest.ts draws), and the entry's own `timestamp` as `lastObservedIso` (event time, never poll-arrival time). An entry with no `backend_service_name` — an LB-synthesized 502/503 that never reached a backend — is dropped honestly rather than attributed to a service it never touched. Resolution runs in two tiers:

1. **Route grain (the fused win).** The GCP `backend_service_name` maps through a config-time `backendServiceMap` to a NEAT service name (resolved once at setup, never inferred at poll time — a backend service name need not match `package.json#name`). When the request's normalized `(method, path)` matches a `RouteNode` that service already declares, the OBSERVED `CALLS` edge lands on that route — and, because the RouteNode records its own definition file/line, the shared pipeline (`routeCallSiteFor`, ADR-143) sharpens the edge to that static site. This is file precision drawn from the *static* route definition, not a runtime stamp.
2. **Backend grain, honestly (the missing-extracted divergence).** When no static route resolves, the connector does not fabricate a route. It declares an honest fallback via `ensureInfraNode` (ADR-133 §4a): the edge lands on `infraId('gcp-lb-backend', <backend_service_name>)`, the LB backend service as the real platform resource its own log names, surfacing as a `missing-extracted` divergence — observed production traffic the codebase's static route table doesn't account for — instead of a silent drop. The fallback id is reserved so a *future* static recognizer for GCP load-balancing / backend services fuses onto the same node rather than twinning it.

Route/backend grain is the honest ceiling, and it is the correct one: an un-instrumented backend emits no `code.*` file-grain telemetry, so the only file precision available is a route's own static definition site (tier 1), and everything else is honestly backend-grained (tier 2). Local and hosted profiles run the identical pull/map/fuse implementation, differing only in how the scoped token is brokered and in poll cadence.

### Consequences

- LB request traffic lands on the same `RouteNode`s static extraction already builds, so a declared route and its production traffic fuse without a GCP-LB-specific node kind — and a route that only production hits, never the static picture, surfaces as a divergence rather than vanishing. This reaches the GKE/Compute-Engine tier the Cloud Run connector structurally cannot.
- Where an LB fronts a Cloud Run service, this connector and the Cloud Run connector observe the same requests, and both mint onto the same `RouteNode` identity — so a project running both against one path fuses rather than twins. The operator picks one per service path: the LB connector is the read for a GKE / Compute Engine backend (which has no per-service request log), the Cloud Run connector for a plain Cloud Run deployment. `docs/connectors/gcp-lb.md` documents the choice.
- The `backend_service_name` → NEAT service mapping is a required piece of setup for route-grain fusion. An unmapped backend still produces honest backend-grained edges (tier 2 falls back to the backend's own name), so a connector added before the map is filled is never silent — it just stays coarse until the map lands.
...
## ADR-220 — Behavioral-failure divergence: a declared dependency that production observes failing is a divergence, not just a missing edge

**Status:** Accepted. Refs #1099. Amends [`divergence-query.md`](contracts/divergence-query.md) (a seventh divergence type; binding rule 4's incident input) and builds on [ADR-215](#adr-215--symbolfield-grain-divergence-an-observed-error-naming-a-missing-member-fused-to-the-code-that-declares-the-access) (the incident-fusion machinery), [ADR-216](#adr-216--stacktrace-code-locus-recovery-an-exception-span-with-a-stacktrace-but-no-code-attributes-still-localizes-to-the-declaring-fileline) (the recoverable locus) and [ADR-158](#adr-158--observed-first-edges-static-first-nodes-symbol-grain-under-file-first-and-the-providerplatformframeworklanguage-agnostic-deterministic-trace) §6 (agnosticity).
**Contract:** [`divergence-query.md`](contracts/divergence-query.md).

> ADR numbering note: 220 is next-free — the tail on `main` is ADR-219 (#1096). Reconfirm against `docs/decisions.md` tail before merge.

### Context

`get_divergences` is the one query whose whole job is *"where does what the code declares disagree with what production observes."* It answers that by comparing **presence**: a declared edge with no observed twin (`missing-observed`), an observed edge with no declared twin (`missing-extracted`), the version/host/compat rules, and — since ADR-215 — a declared member access the runtime object does not have (`observed-symbol-mismatch`). A whole class of divergence sits in none of those buckets and so falls straight through: a dependency the code declares **and** production observes, but whose calls **predominantly fail**. The edge is present, so it is neither `missing-observed` nor `missing-extracted`; the access is fine, so it is not a symbol mismatch; yet the declared intent "this dependency works" and the observed reality "it fails every call" are as far apart as a divergence gets. And `incidents` — the store that already carries the failures — were read only for the narrow symbol/attribute-error path, never for connection/deadline/timeout/5xx failures.

The ±NEAT divergence bench on the live OTel Demo made the gap concrete and measurable. Across the divergence-shaped PRAXIS faults, `neat divergences` fired on ~0/13 while the neat *agent*, working the same fused graph, hit 96% — because it pivoted to `observed-dependencies` / `incidents` / `stale-edges`, which already carry the failure signal. Three concrete misses: `frontend → ad` observed with 84/84 error spans (`observed-dependencies` shows it; `divergences` did not — the edge is "observed", just all-erroring); `recommendation` pointed at a wrong database host, its incident reading `ECONNREFUSED`, never folded in; and the neo4j call observed hanging / `DEADLINE_EXCEEDED`, known through incidents, not surfaced. This is a **verb wiring gap, not a capability gap**: the fused graph has the failure; the `divergences` verb did not fold it in.

### Decision

1. **A seventh divergence type, `observed-failing`, at two loci.** It surfaces a declared dependency whose observed behaviour is failure, additive schema growth on `DivergenceSchema` (`schema.md` / ADR-031: a new discriminated-union variant + a new `DivergenceType` value, no `SCHEMA_VERSION` step). The **edge locus** is a declared+observed dependency edge whose observed error rate is over the threshold; `source`/`target` are the edge endpoints and it carries `edgeType`, the `observed` edge, and the `spanCount`/`errorCount`/`errorRate` evidence. The **incident locus** is a declared external call whose recorded incident indicates a transport/5xx failure, fused to the declaring `file:line` exactly as ADR-215 fuses a symbol mismatch; `source`/`target` are both the code node (mirroring the symbol locus), and it carries `location`, the fused `incidentId` + `errorMessage`, an `incidentCount`, and the `httpStatusCode` when the failure is an HTTP one. A single `failureKind` field names the generic semantic that classified it.

2. **Both loci key on generic failure *semantics*, never a provider/framework/host/language name** (the ADR-158 §6 invariant `divergences.ts` is already scanned for). The failure kinds are a closed, neutral set: `error-rate` (the edge locus), and the incident-locus transport families `connection-refused`, `deadline-exceeded`, `timeout`, `unavailable`, and `server-error`. The transport families are matched from the error text against cross-language semantics — a gRPC/status name (`DEADLINE_EXCEEDED`, `UNAVAILABLE`), a POSIX errno (`ECONNREFUSED`, `ETIMEDOUT`, `ECONNRESET`), and their plain-English equivalents — so a new phrasing joins a family and a new language never gets a branch. A 5xx is classified structurally off the incident's `httpStatusCode` (500–599), never a message substring, so a literal like "500ms" never trips it; a 4xx is a client-side concern and is left out. To the detector, an erroring gRPC edge, a refused TCP connect, and a 503 are one fact — a declared dependency that does not work.

3. **The edge locus fires only on a declared+observed edge that is predominantly failing, and never on a merely-degraded one.** Both sides must be present: an observed-only failing edge is already the headline `missing-extracted` finding (and has no *declared* intent to diverge against), and a declared-only edge has no observed calls to fail, so neither is re-flagged. "Predominantly failing" is an error rate at or above **0.5** over an edge exercised at least **5** times. Half is the defensible line between *degraded* — a working dependency erroring on a minority of calls, which is an SLO / alerting concern, not a declared-vs-observed divergence — and *broken*, where the declared intent is not being realized for most calls; below it the dependency is still succeeding for the majority, so it stays off this surface. The 5-span floor keeps a one-off or low-volume blip (a single 1/1 error) from reading as a pattern; the span/error counts ride on the finding so an operator sees the sample size. This holds the existing false-positive discipline: a healthy edge, even a flaky one, never trips 50%.

4. **The incident locus is sourced from the incident store, fused to the declaring code, and still pure.** The wrong-host / connection-refused and deadline cases mint no successful observed edge — the call fails before it completes — so their only record is the incident, localized to `code.filepath`/`code.lineno` (ADR-215's join, made recoverable from a stacktrace by ADR-216). `computeDivergences` reads them from the same in-memory `opts.incidents` array the symbol detector already receives; the read of the append-only sidecar stays at the REST/CLI/`ask` call site, so the function stays pure — no I/O, no mutation, no async. Absent or empty incidents leave the edge-grain result byte-identical. Repeated incidents of one (locus, family) collapse to a single finding carrying the count, the same "count the failure mode, not every incident" discipline the root-cause localizer and the symbol detector use.

5. **Graded INFERRED, ranked below the definitive structural and symbol divergences.** The failure itself is observed for certain; the claim that it is *the declared dependency* diverging is a cross-layer join, so the finding carries the ~0.6 INFERRED grade — the same grade a stitched edge, an incident-localized root cause, and an ADR-215 symbol finding take. At 0.6 it sorts beneath the definitive `version-mismatch`/`compat-violation` (1.0) and the structural `missing-extracted`, and well above a dampened dead-code probe (ADR-213, 0.1); the type-leadership tiebreak places it last, so at equal confidence a structural or symbol divergence still leads. The edge and incident loci are complementary readings — the failing edge as the caller sees it, and the declaring call site with the runtime's own reason (a connection-refused call that mints no edge can only be named from the incident) — and each de-duplicates within itself, so counts stay honest.

### Consequences

- The `frontend → ad` all-erroring edge, the `recommendation → wrong-host` `ECONNREFUSED`, and the neo4j `DEADLINE_EXCEEDED` / hang class now surface through the query a user actually runs to find them — pointed at the failing dependency edge or the declaring `file:line`, ranked, with the observed error evidence attached. The verb catches up to the graph: the failure the agent already found through `incidents`/`observed-dependencies` is now folded into `divergences` itself.
- The divergence surface becomes the one-stop *"code↔runtime disagreement"* view at another grain it can compute — declared-and-observed-but-failing — alongside missing edges, column drift, and symbol/field access. The taxonomy grows by one deliberate, ADR-gated type; it does not become a per-error catalogue, because the failure kinds are generic and closed.
- False-positive discipline is preserved by construction, not by exception: a healthy or merely-degraded edge is below the 0.5 rate, a low-volume edge is below the 5-span floor, an observed-only edge is left to `missing-extracted`, and an incident that is a 4xx, a member mismatch, or a benign record is not a behavioral failure and is not classified. Nothing on the existing five types or the ADR-215 sixth changes; the new type only adds findings the surface was dropping.
- The one honest limit is the OBSERVED layer's own: this reads the failures production actually recorded — an un-instrumented dependency, or a failure that never fired in the window, produces nothing, exactly as the observed layer is silent where it has not seen. It is a recall bound of observation, not a false claim; `incidents` and `observed-dependencies` remain the complementary raw views.

## ADR-221 — The incident card: a self-sufficient work order pushed on every incident (a ninth SSE event type)

**Contract:** [`docs/contracts/incident-card.md`](contracts/incident-card.md) (new). Amends [`frontend-api.md`](contracts/frontend-api.md) (the locked event taxonomy grows to nine) and [`cli-surface.md`](contracts/cli-surface.md) (the monitor gains an `incident` fact kind). Builds on the root-cause traversal (ADR-189/190), blast radius (ADR-038), the policy overlay (ADR-108), the divergence query (ADR-060/215), stacktrace locus recovery (ADR-216), the SSE bus (ADR-051), and the live-context monitor (ADR-159/162).

### Context

An incident already leaves a complete record. A failing span becomes an `ErrorEvent` in the append-only incident store (a 5xx, a coalesced 4xx burst, an exception with type and stacktrace, a connector failure — ADR-185), carrying the affected node and, since ADR-216, a recovered `code.filepath`/`code.lineno`. Everything an agent needs to *act* on that incident is computable from the graph: the causal chain (`get_root_cause`), what a fix would reach (`get_blast_radius`), the rules that govern the file about to change (the policy overlay), and whether code already disagrees with runtime at that node (`get_divergences`). But those are pull-only and uncomposed — an agent gets them only if it thinks to ask, one query at a time, and stitches them itself. And the one surface that *pushes* live facts to an agent, the SSE `/events` bus, has no incident type: the monitor (ADR-159/162) turns structural events into pushed lines, but incidents are the single high-signal fact it cannot carry. So an agent learns of a production failure by luck, and reconstructs its context by hand — the exact context-hunting a fused graph exists to remove.

### Decision

1. **The incident card — one composed shape.** `IncidentCardSchema` (new, in `@neat.is/types`) fuses the `ErrorEvent` with the queries that already ship: the incident (kind, time, affected node, message), the recovered `locus` (or `null`), the `rootCause` with its causal `chain` annotated per hop, the `blastRadius`, the governing `policies`, any `divergence` at the node, and a rendered `headline` sentence. It is the agent's work order — where the cause is, what the fix reaches, what governs the file, and how much each claim can be trusted — assembled once so the agent reads instead of grepping.

2. **A ninth SSE event type, `incident` — the successor the ADR-051 lock requires.** The eight-type taxonomy was locked so the bus would not sprawl with derivable or noisy signals; divergence was deliberately kept off it because it is re-derivable from current graph state at any time (a pure query, no persistence — ADR-060). An incident is the opposite: an append-only, point-in-time fact that is *not* reconstructable from the graph's present state, so no structural trigger can stand in for it. That is precisely the fact a push bus is for, which makes this a principled break of the lock, not sprawl. The payload is lean — a trigger, not the context: `{ incidentId, affectedNode, service, incidentKind, at }`. The full card is a REST read, keeping the "trigger thin, context fat" shape the monitor already relies on.

3. **One composed REST read backs both surfaces.** `GET /graph/incident-card/:nodeId` (dual-mounted per ADR-026, optional `?errorId=` to pin one incident; default is the node's most recent) returns the card. It backs the `get_incident_card` MCP tool (the pull path — an agent that asks) and the monitor's `incident` fact kind (the push path — the monitor reads the card for the id its `incident` event carried). The assembler is pure composition over already-governed queries; it computes nothing new, reads read-only, and bounds its incident read the same way ADR's #1083 discipline bounds `get_incident_history`.

4. **The card is zero-fabrication with calibrated provenance — never "100% fact."** A missing locus is `null`, not an invented `file:line`; every chain hop carries its own `provenance` and `carriesSignal`, so an OBSERVED link and an INFERRED stitch are never flattened together; a saturated downstream victim is labeled `symptom-only`, not dressed as a cause. The value is not that every claim is certain — it is that the agent stops inventing context and starts reading it, and knows the edges of its own knowledge instead of guessing across them.

5. **The monitor gains an `incident` fact kind, and the locked surfaces grow to nine.** `neat monitor` emits one `✖ incident […]` line per new incident (and the full card under `--json`), triggered by the new event. The taxonomy grows by one deliberate, ADR-gated type — the first addition since the ADR-051 lock — amended in `frontend-api.md`, `api-reference.md`, and `cli-surface.md`. The card type is additive schema growth (ADR-031): a new exported schema, no shape change to any existing one.

### Consequences

- An agent wired to `neat monitor --json` wakes on every incident with a complete work order and can begin the fix without a single context query; an agent that prefers to ask gets the identical card from `get_incident_card`. The context-hunting a static index forces — and that a snapshot graph has no events to push for — is replaced by a pushed, provenance-stamped fact.
- The bus taxonomy grows for the first time since it was locked, and the justification is the discipline that keeps it honest: only a non-reconstructable, high-signal fact earns a type. An incident qualifies; a derivable query does not, so divergence and the rest stay off it.
- The card is the `propose`-input shape the autonomous-remediation runner (ADR-106) will consume when it lands — building it now lays that brick without pulling the runner forward.
- The card is only as good as the layers beneath it. Detection that is blind to attribute-carried gRPC/HTTP status errors (#1065) and root-cause victim-navigation that can misname a load origin (#1075) bound what the card can carry — but it degrades honestly, surfacing a `null` locus or a `symptom-only` verdict rather than fabricating, so the ceiling is recall, never a false claim.

## ADR-222 — A boundary timeout is a symptom, not the fault: classify it symptom-only and point structurally upstream when the culprit is unobservable

**Contract:** amends [`get-root-cause.md`](contracts/get-root-cause.md) / traversal (the classifier and the candidate walk) when the code lands. Builds on the node classifier (ADR-189), the load-origin walk (ADR-189), victim-navigation (ADR-214, #1075), the behavioral-failure failure-kinds (ADR-220), and the incident card (ADR-221). This ADR fixes the design; the classifier change lands in its own PR.

### Context

The card's root cause is only as good as the classifier beneath it, and the ±NEAT grind found the one class of fault where the classifier confidently names the wrong node: a **hang**. When a downstream service hangs with no timeout of its own, its span never ends, so it **never exports** — and the gRPC/HTTP client call that is blocked waiting on it never ends either. The only span that exports in the whole stuck chain is the one at a boundary that has its *own* request timeout (an Envoy/ingress proxy returning a 504). NEAT sees that boundary 504 and nothing beneath it, so `classifyNode` (a node that emits an error of its own, with no erroring callers, that is not stale/saturated → `primary-failure`) names the **gateway** the primary failure at 0.6 and points the agent at the wrong service.

This is verified, not inferred. On the demo's hang scenario, the collector's own debug exporter — reading spans *before* any NEAT ingest gate — received **zero** spans from the hung culprit and zero from its blocked caller, while capture was provably working elsewhere in the same window. So NEAT drops nothing (it is not an `isError`-gate or long-running-span artifact); the culprit genuinely emits nothing. The contrasting cases sharpen the line, and both are the classifier working correctly: a service that **errors** exports a span (an exception, or a fast-fail like a scaled-to-0 service returning `UNAVAILABLE`, or `ECONNREFUSED`), and there NEAT walks to the real culprit. The dividing line between "NEAT sees it" and "NEAT is blind" is **error-vs-hang emission** — and a boundary timeout is, by definition, the observable shadow of something unobservable downstream.

### Decision

1. **A boundary timeout classifies `symptom-only`, not `primary-failure`.** A new branch in the classifier fires when a node's own error is a **gateway/timeout-class** failure — the `deadline-exceeded` / `timeout` / gateway `server-error` (504) families the failure-kind classifier already names (ADR-220) — **and** the node has **declared outbound dependencies**, **and** it has **no erroring callers**, **and** — the load-bearing clause — it has **no observed erroring downstream edge**. Such a node is not failing of its own logic; it is timing out waiting on something it can't see. It is the same "emits an error but is really a victim" shape ADR-189 already carves out for a saturated node, extended to the boundary-timeout case where the victimhood is structural rather than load-driven.

2. **The last clause keeps the observable cases correct.** When a downstream failure *is* observed — a scaled-to-0 `UNAVAILABLE`, an `ECONNREFUSED`, any fast-fail that exports an erroring edge — the classifier must keep walking to the real culprit exactly as it does today (verified: the grind's scaled-to-0 run root-causes the failing service correctly off a 50/50 observed error edge). So the reclassification fires **only** when there is genuinely no observed erroring downstream edge to walk. Without this clause the boundary-timeout branch would swallow the cases that already work; with it, the branch is scoped to precisely the blind case.

3. **The signals ride on the classifier's context, not new I/O.** `classifyNode` stays a pure function of a `NodeContext` (ADR-189). The two new signals — the node's own error is gateway-timeout-class, and whether any outbound OBSERVED edge is erroring — are computed in the context builder that already holds the graph and the incident set; the classifier reads them the same way it reads `errorsEmittedHere`. No ingest change, no new query.

4. **When the culprit is unobservable, point structurally upstream at INFERRED confidence.** A boundary node reclassified `symptom-only` through this path has, by construction, no observed downstream to walk — so navigation instead walks the **EXTRACTED** call graph outward and emits a candidate that names the declared path (`frontend-proxy → frontend → recommendation → neo4j`) as the likely locus of the cause, graded **INFERRED / low confidence** (the EXTRACTED provenance ceiling, 0.5). This mirrors the load-origin promotion (ADR-189) but sources the walk from declared edges when the observed layer is blind. An honest "the cause is upstream along this declared path, I can't see which hop" beats confidently naming the gateway.

5. **The coverage bound is inherent and stated, never fabricated.** A span that never exports cannot be observed; NEAT does not invent the culprit's runtime. The card degrades to a `symptom-only` boundary plus a structural-upstream pointer — a recall bound of the OBSERVED layer, not a false claim. This is the RCA-quality complement to ADR-221's card-locus promotion: #1112 makes the card point at the cause's *code* when the cause is seen; this makes the card *honest* when the cause is unseen.

### Consequences

- The hang card stops naming the boundary the primary failure. It reads `symptom-only` with an INFERRED pointer up the declared call graph — the agent is sent upstream toward the real fault instead of at the gateway. The confidence drops from a misleading 0.6 to a graded INFERRED that says "structural guess, not observed." Measured baseline this fixes (grind, 0.9.9, clean hang run): **32/32 hang cards land on `service:frontend-proxy` classified `primary-failure` at 0.6, with `locus: null`** — the locus is correctly null because the boundary is a service node with no code for ADR-221's promotion to point at. So #1112 already makes the *locus* half honest (null, never fabricated); ADR-222 makes the *classification* half honest, turning that confident-wrong gateway verdict into `symptom-only` + a structural-upstream pointer.
...
## ADR-224 — Kubernetes is a deployment substrate: an observed cluster-state reader, enabled off the vendor connector surface

**Status:** Accepted. Refs #1124. Amends [`connectors.md`](contracts/connectors.md) §10 and [`connector-config.md`](contracts/connector-config.md); adds [`docs/connectors/kubernetes.md`](connectors/kubernetes.md). The observed leg of a first-class k8s substrate; the declared manifest extractor and the declared/observed divergence are its other legs, named here and built next.

### Context

The graph goes blind when a service is down for a *deployment* reason — a bad image, zero replicas, a crashloop — because the dead pod emits no spans. A full-suite bench made it concrete: on `product-catalog` ImagePullBackOff, `ad` scaled to 0, and `recommendation` crash/hang, graph-only NEAT scored 0–2/5 while an agent with `kubectl` read the cause straight from cluster state and scored 5/5. So NEAT needs to pull cluster state into the graph.

The first framing was "Kubernetes is another connector to pull from," beside Supabase / Vercel / Neon. That is a category error. Kubernetes is not a SaaS vendor — it is ubiquitous deployment infrastructure, one API GKE / EKS / AKS / kind all speak. Two properties make it want first-class *substrate* treatment rather than a vendor slot:

1. **It has a declared side.** The repo's manifests carry the desired image and desired replicas. So Kubernetes is a divergence source — declared image vs running image, declared replicas vs ready — which is NEAT's core value. No vendor connector can tell that story; a vendor has no declared twin.
2. **The pull is a bridge around NEAT's own limit, not a vendor integration.** The ubiquitous path to cluster state is the OTel Kubernetes receivers (`k8sclusterreceiver` / `k8sobjectsreceiver`); the only reason NEAT pulls the API directly is that NEAT ingests spans, not the metrics/events those receivers emit — the exact span-only limit the bench exposed. The reader is a pragmatic bridge, valuable until NEAT ingests those signals.

### Decision

Kubernetes lands as a **deployment substrate** with three legs: DECLARED (a static manifest extractor — desired image / replicas / workload→service — a follow-on in the extractor lane), OBSERVED (this ADR's live cluster-state reader), and the DIVERGENCE join between them (the payoff, and the reason k8s is first-class). This ADR ships the OBSERVED leg and names the other two.

**The observed reader is not a `neat connector` vendor.** It has no `PROVIDER_DISPATCH` entry and no `neat connector add` surface. It is enabled off a dedicated config, `~/.neat/k8s.json` (machine-level, the same env-ref-by-default / `0600` / no-secret-at-rest discipline `connectors.json` holds), which the daemon reads at slot bootstrap (`startK8sSubstratePolling`, wired beside `startConnectorPolling`) and runs through the **reused** connector plumbing — `startConnectorPollLoop`, the shared junction, and the incident pipeline. The reader's code (`connectors/kubernetes/`) reuses that plumbing deliberately; what changes from a connector is the enable surface and the framing, not the mechanism.

The reader `poll()`s Deployments + Pods in one namespace (read-only, `get`/`list` RBAC only), classifies each unhealthy workload, and emits a `ConnectorIncident`-bearing signal the shared pipeline writes onto the workload's `ServiceNode` — the same incident-emitting shape EAS introduced (§10), the same fused-service lookup (`resolveFusedServiceId`) so the incident lands on the extractor's node, never a twin. The faults: `ImagePullBackOff` → "cannot pull image `<tag>`"; `spec.replicas: 0` → "scaled to 0"; `CrashLoopBackOff` → "crashlooping … last terminated `<reason>`"; a down-but-uncaused deployment → an honest `no-ready-replicas`. A healthy workload mints nothing. The dedupe id is stable per `(namespace, deployment, fault)` — a current-state fact — so a re-polled fault collapses to one incident. The cluster API's CA-signed / client-cert TLS (which the platform `fetch` won't build from a PEM) is handled by a Node `https` agent wrapped as a `fetchImpl` through the junction — no k8s SDK, no new dependency (the `yaml` dep already in the tree parses the kubeconfig).

**Two seam decisions the declared and observed legs share** (joint calls, so both legs and the divergence join build to the same shape):

1. **Deployment node ids are name-only** — `infraId('k8s-deployment', metadata.name)`, with the namespace carried as a `namespace` attribute rather than in the id. A manifest frequently omits `metadata.namespace` (it is supplied at `kubectl apply -n` time — the OpenTelemetry Demo manifest does exactly this), so the declared side cannot always know the deploy-time namespace, while the observed side always can; the only key both legs reliably produce is the name. Name-only also matches NEAT's own identity model — a `ServiceNode` is `service:<name>` (ADR-010, one node per name) — so the deployment node, the service node, and the incident all key on the same bare name. The cross-namespace name-collision this admits is the one NEAT's `ServiceNode` already carries, not a new exposure.
2. **The declared⋈observed compare home is `service:<name>`.** The declared extractor stamps `declaredImage` / `declaredReplicas` (EXTRACTED) on the service node; the observed reader stamps `observedImage` / `observedReadyReplicas` (OBSERVED) on the same node; the divergence detector (ADR-225) is a node-local compare of those four. The service node is chosen because it is already name-only (so the namespace-ambiguity of point 1 does not arise for the compare at all), it is where the incident already lands, and both provenances meeting on one node is the two-sided-fusion pattern. The observed running-image is stamped for **all** workloads (healthy included), not read from an incident — so a *stuck rollout* (declared image ≠ running image, service still up on the old ReplicaSet, no outage, no incident) surfaces as a pure divergence, which an outage-only reader by construction cannot see. The `RUNS_ON` edge `service:<name> → infra:k8s-deployment:<name>` carries topology/blast-radius, not the compare.

**Live-verified before this decision.** On a kind cluster running the OpenTelemetry Demo with the three faults injected, the reader minted the correct `k8s-deploy-failure` incident on the correct extracted service node for each: `product-catalog` → "cannot pull image app-image:latest (ImagePullBackOff)", `ad` → "scaled to 0", `recommendation` → "crashlooping (restarts: 4); last terminated: Error (exit 1)". No `serviceMap` was needed — the demo's deployment names matched the extracted node names. Two findings shaped the design: an intentionally-scaled-0 sidecar (the demo's load-generator) motivated an `expectedZero` allowlist so an expected zero is not noise; and a *stuck rollout* (bad image that never took, old ReplicaSet still serving) correctly minted nothing — no outage — which is exactly the declared-image ≠ running-image divergence the declared leg + join must cover, the strongest evidence for substrate over connector.

### Consequences

- The three bench faults become first-class OBSERVED incidents on the service node, so an agent root-causes a deploy outage from the graph instead of reaching for `kubectl` — the blind spot closed, without instrumenting the workload.
- Kubernetes is not listed beside SaaS vendors, so the graph and the docs do not tell the category-error story "k8s is a vendor." The reader reuses the connector plane's plumbing while being enabled off a separate substrate surface.
- The observed reader sees outages, not silent bad-deploys; the stuck-rollout case is deferred to the declared leg + divergence, which is where a manifest twin makes it visible. That split is the substrate's reason to exist and the next work: a manifest extractor (`extract/infra/kubernetes.ts`, the recognizer lane) and a declared-vs-observed divergence.
- `~/.neat/k8s.json` is a new machine-level config surface. It reuses `connectors.json`'s env-ref resolution and read discipline; a full `neat k8s` write/CLI surface (parallel to `neat connector`) is a follow-on — for now the daemon reads the file and the entry is authored directly, the same way the connector read-path landed ahead of its CLI.
- The reader is a bridge around NEAT's span-only ingest. If NEAT later ingests the OTel k8s receivers' metrics/events, that becomes the ubiquitous observed path and this reader can retire behind it; the declared leg and the divergence — the first-class value — do not depend on which observed path feeds them.

## ADR-225 — The k8s deployment substrate's divergence: declared image ⋈ observed running image on the service node

**Status:** Accepted. Refs #1124. The third and capstone leg of the k8s deployment substrate (ADR-224 observed, the manifest extractor declared), and the reason it earns first-class treatment. Amends [`divergence-query.md`](contracts/divergence-query.md) (a new `deploy-mismatch` divergence type) and [`schema.md`](contracts/schema.md) (two `ServiceNode` fields); touches [`connectors.md`](contracts/connectors.md) and [`docs/connectors/kubernetes.md`](connectors/kubernetes.md).

### Context

ADR-224 named three legs — declared (a manifest extractor), observed (a live cluster reader), and the **divergence** join between them — and shipped the observed leg. The declared leg landed alongside it (`extract/infra/k8s.ts` stamps `declaredImage`/`declaredReplicas` on the `service:<name>` node). This ADR is the join, and it is the payoff: the observed reader sees *outages* (a pod actually down mints an incident), but a *stuck rollout* is not an outage. When a manifest declares a new image and the apply does not take — the old ReplicaSet keeps serving, `readyReplicas` stays healthy — the service is up, so no pod fails, no span is absent, and no incident fires. Yet the deploy silently did not land: declared image ≠ running image. An outage reader cannot see this by construction, and a vendor connector cannot either (it has no declared twin). The live bench verify surfaced exactly this case as the one the incident reader correctly stays silent on.

### Decision

The divergence is a **node-local compare on the `service:<name>` node**, where both provenances already meet (ADR-224's joint seam decision): the manifest extractor stamps `declaredImage`/`declaredReplicas` (EXTRACTED), and the observed reader now also stamps `observedImage`/`observedReadyReplicas` (OBSERVED). A new `deploy-mismatch` divergence type (`@neat.is/types`), added to the locked `DivergenceType` allowlist and the `get_divergences` surfaces, is emitted by `detectDeployDivergence` in the ServiceNode pass of `computeDivergences` when the two disagree. `kind: 'image'` — the headline — fires when `declaredImage !== observedImage`: the stuck rollout, high-confidence (both sides are exact facts), with a reason that names both images and notes no incident fires. Both sides must be present to compare, so a workload with no manifest or no running pods is not a divergence.

The observed side is recorded through the connector pipeline, not a connector mutation (ADR-030): the k8s reader emits a **deploy-state signal per workload — healthy included** (`callCount: 0`, no incident, `deployState: { image, readyReplicas }`), and `runConnectorPoll` stamps it onto the resolved service node via a new `mergeObservedDeployState` ingest primitive — the same additive-node-attribute path `columns` takes through `mergeObservedColumns`. The running image is read from a *running* pod's container (for a stuck rollout, the old ReplicaSet's image), not from the deployment's own template, so it is genuinely what the cluster serves. Recording it for every workload — not only via an incident — is what lets the divergence fire when there is no incident at all.

`kind: 'replicas'` is reserved in the schema but not emitted in this cut: `declaredReplicas !== observedReadyReplicas` fires transiently during any rolling update or scale, and the fully-down case is already the `no-ready-replicas` incident. A stable, debounced replica divergence is a named follow-on; the `observedReadyReplicas` stamp it needs is already recorded.

### Consequences

- The substrate's first-class value lands: an agent asks `get_divergences` and sees "your deploy did not take — declared image X, running Y" for a service that is up and emitting nothing wrong. This is the pure fusion win — invisible to a code-only reader and to an outage-only reader, decisive once declared and observed meet on one node.
- Two `ServiceNode` fields (`observedImage`, `observedReadyReplicas`) and one divergence type are additive schema growth (ADR-031), no migration. The `get_divergences` allowlist gains one member, explicitly, the same way ADR-215 and ADR-220 added theirs.
- Both legs shipped in 0.9.12 as data; this capstone turns that data into the query. It reads `declaredImage` gracefully when absent (a non-k8s service, or before the declared extractor has run), so it is a no-op until both sides exist — no ordering hazard.
- The replica divergence is deferred rather than shipped noisy: a transient rollout must not read as a standing divergence. The high-signal image compare is the one that carries the substrate's thesis, and it is stable (a stuck rollout persists until fixed).

## ADR-226 — FRONTIER is the graph's edge: a promotable surface for what sits outside the settled graph (kernel step 1, the hang as first user)

**Status:** Accepted. The first step of the governance kernel (ADR-093), landed on the read/observe side ahead of the write/gate side. Builds on FRONTIER-the-reserved-value (ADR-094), FrontierNode + its promotion (ADR-023/044/068), the boundary-timeout classifier (ADR-222), and the node classifier / candidate walk (ADR-189).
**Contract:** amends [`provenance.md`](contracts/provenance.md) (FRONTIER gains present-tense write semantics + the promotion path), [`contracts.md` Rule 3](contracts.md) (FRONTIER *edges* excluded from settled traversal, beside the existing FrontierNode gating), [`frontier-promotion.md`](contracts/frontier-promotion.md) (promotion generalizes node→edge), [`get-root-cause.md`](contracts/get-root-cause.md) (the hang reader). Build sequence: the missing `docs/plans/2026-06-09-governance-kernel-build-ladder.md`, first rung.

### Context

FRONTIER has a tangled history across three documents, and the resolution is to unify them, not pick one:
- **PROVENANCE.md (earliest):** FRONTIER = a *placeholder* — "we know a dependency exists because logs name it, but we have no node for the other end," ranked at the floor (`… STALE ≥ FRONTIER`). The **unobservable present**.
- **ADR-068:** *retired* FRONTIER-as-provenance to end the FrontierNode-vs-provenance confusion; froze the enum at four; unresolved peers became the `FrontierNode` node-type (edges *to* them stay OBSERVED).
- **ADR-094:** *re-reserved* FRONTIER as the **staged-proposal / future** tense — kernel-written, gated — stating outright the enum "carries five values … FRONTIER reserved-but-unwritten."

These are two faces of one thing: **FRONTIER is the label for what sits outside the settled graph.** The four settled provenances (OBSERVED/EXTRACTED/INFERRED/STALE) *are* the graph — parsed, observed, stitched. FRONTIER is its edge, facing two ways: the **unobservable present** (an external API, or a hanging service where NEAT "shot OTel into the universe" and nothing came back) and the **not-yet-real future** (a staged feature env, a would-be deploy). Both await **graduation to OBSERVED**; both are, until then, outside the graph.

The trigger is the **hang** (ADR-222). When a downstream service hangs, its span never exports and neither does the client call blocked on it — the only span is a boundary 504. ADR-222 correctly reclassifies that boundary `symptom-only`, but its §4 pointer to the culprit was graded **INFERRED** — laundering a *guess about a thing NEAT cannot see* as a settled stitch. The honest label for "a hop we believe exists but cannot observe" is FRONTIER, and — the load-bearing insight — a hang **promotes to OBSERVED the moment the service stops hanging** and its spans return. That is the same graduation FrontierNode already performs (ADR-044): a placeholder swapped for the real thing once it resolves.

### Decision

1. **FRONTIER is a real edge provenance again — a *surface*.** The fifth `Provenance` value lands (resolving ADR-094's reservation), and a **FRONTIER edge** may exist in the graph: a placeholder for an OBSERVED edge NEAT cannot yet see. This restores `frontierEdgeId` (retired by ADR-068) as the canonical id helper. `FrontierNode` is the *node* case of this same idea (a placeholder peer); a FRONTIER edge is the *edge* case (a placeholder hop) — unifying them is deferred (a later fold-on), but the surface concept is one.

2. **FRONTIER stays out of `PROV_RANK` and settled traversal.** A proposal is not part of the real graph, so a FRONTIER edge is never ranked against a settled edge and is **skipped** by `getRootCause` / `getBlastRadius` / dependencies — the edge-level twin of Rule 3's node-level "stop at FrontierNodes." `PROV_RANK` keeps its four settled entries; the edge `ProvenanceSchema` gains FRONTIER so a FRONTIER edge validates and persists, but every ranking/traversal site guards it out. This is the exact split ADR-094 anticipated: FRONTIER gains write semantics without entering the settled contest.

3. **Two graduation mechanisms, one tense — this sharpens ADR-093.** ADR-093 routes settled→flag, FRONTIER→gate. That is the **future face**: a proposed deploy is *preventable*, so it graduates through the policy **gate** (block refuses it). The **present face cannot be prevented** — a hang is a fact you could not see, not a change you might stop — so it graduates by **promotion**: the FRONTIER surface is replaced when its OBSERVED twin arrives. Sharper rule: **proposed-change FRONTIER gates (preventable); unseen-reality FRONTIER promotes-on-observe (only discoverable, never blocked).** This step builds only the promotion path; the gate is a later rung.

4. **The hang is composed from general primitives — no "hang" block (the redstone rule).** There is no bespoke hang subsystem. A hang is the wiring of four general blocks: (a) the **FRONTIER surface** (edge), (b) **promotion** on OBSERVED arrival, (c) a **sensor** — a write-time analysis pass that stages a FRONTIER edge on the boundary-timeout-over-unobservable-downstream signature (post-ingest, never the OTLP hot path — ADR-093 keeps that untouched), (d) a **reader** — `get_root_cause` surfaces the staged surface as the proposal, ranked below every settled cause. Nothing says "hang" except the sensor; every other block is general and reused.

5. **The FRONTIER edge coexists with the declared edge; it *is* the OBSERVED edge's placeholder.** The declared `frontend→recommendation` hop is EXTRACTED (static). The FRONTIER hop is the *runtime* call NEAT reached toward but cannot observe (the far end hung). They coexist the way OBSERVED and EXTRACTED already do, and the FRONTIER runtime edge promotes to the OBSERVED runtime edge when the service recovers. No redundant edge — the FRONTIER edge is precisely the placeholder for the OBSERVED one.

### Consequences

- ADR-068's four-value freeze is superseded on the terms ADR-094 set: the enum and edge `ProvenanceSchema` become five; `PROV_RANK` and the edge-orthogonality invariants stay four/intact; `frontierEdgeId` returns; the "no `Provenance.FRONTIER` reference" guard is retired for the kernel's own use. The audit is rewritten to lock the *new* invariants (FRONTIER out of PROV_RANK and settled traversal), not the old freeze.
- The hang stops being an INFERRED fabrication and becomes an honest FRONTIER surface that *promotes* when the service recovers — the graph tells the agent "I can't see this; here's the hop I reached toward; it'll turn OBSERVED when it comes back." Provenance carries the honesty at the type level, not a reason string.
- FRONTIER ships its first real writer ahead of the kernel's gate — on the **promote** path (present face), not the **gate** path (future face). The kernel's edge-staging proposal channel (ADR-093) and the remediation runner's staging envs (ADR-106) remain later rungs; this introduces no gate and no new enforcement primitive.
- Delivered as a stacked **series** — the FRONTIER-surface primitive (value, id, persist, traversal-exclusion), then promotion, then the hang sensor, then the reader — each atomic and green, composing the hang from general blocks. Deferred: the gate/future-face, the proposal channel, staging-env creation, and the FrontierNode↔FRONTIER-edge unification.

## ADR-227 — `neat config`: a hosted-only terminal setup TUI over the daemon connection (connectors · repositories · web GUI)

**Status:** Proposed. No issue yet. On implementation, amends [`cli-surface.md`](contracts/cli-surface.md) (the `neat config` command) and builds on [`client-profiles.md`](contracts/client-profiles.md) (the daemon connection an active profile encodes), the durable NEAT API key (neat-infra PR #24 — `neat_pat_` auth on `/me`), the token-paste connector seam (neat-infra PR #25 — `GET /me/connectable`, `GET|POST|DELETE /me/projects/:id/connections`), and the hosted control plane. **Proposed, not Accepted:** the seams it consumes are verified, but the command itself is the forward build — it becomes Accepted once implemented and live-verified against a provisioned tenant (per the repo rule that an ADR records evidence that holds).

### Context

`npx neat.is` → `neat login` establishes a durable connection: the login flow writes `~/.neat/profiles.json` with the tenant daemon's endpoint + token (and, per the two-credential decision, the control-plane URL + a `neat_pat_`), after which the CLI and MCP read that hosted daemon by default. In one line: **after you log in, your laptop is connected to the daemon.** That is the whole posture — it should feel like logging into Claude on your terminal.

But once connected, there is no terminal way to *manage* the hosted account behind the connection — what data sources feed the graph, what repositories are synced, where the console is. Today that management lives only in the web console (`app.neat.is`). A terminal-native user who lives in `neat login` has to leave for the browser to connect a database or add a repo. The connect broker (neat-infra `src/connect/`) and its `/me/projects/:id/connections` surface exist; the GitHub-repo sync path (the GitHub App, neat-infra #19) is named but not yet deployed. What is missing is the terminal front-end that ties them into the connection `login` already made.

This management is inherently a **hosted** concern. Connectors are third-party credentials sealed in the tenant's KMS vault on the control plane; repositories are synced into the tenant daemon by the hosted GitHub App; the web GUI is the hosted console. A self-hosted local NEAT has none of these — it has a daemon the user runs and configures directly (files, `neat` verbs). So `neat config` is a managed-hosting affordance, which also matches the business model: the software is free and self-hostable; managed hosting is the paid product, and the hosted setup experience is part of what is paid for.

### Decision

Add **`neat config`** — an interactive, keyboard-driven terminal menu (the shape of the Claude MCP TUI menu) that manages the hosted account behind the current daemon connection. It operates on the **active profile's project** — the daemon the laptop is connected to — and authenticates every control-plane call with that profile's `neat_pat_` (ADR/PR #24).

**Hosted-only gate.** `neat config` requires an active profile that targets a hosted control plane (a CP URL + a `neat_pat_`). When the active target is a self-hosted/local daemon (no control plane), it does not open the menu — it prints one clear line: that `neat config` manages a hosted NEAT account, and the user can `neat login` to a hosted account, or manage a self-hosted daemon directly / through the web GUI they run. This is a deliberate product boundary, not a stub — the three sections all act on control-plane state a local daemon does not have.

**Top-level menu — three entries:**

1. **Connectors** — the terminal twin of the marketplace, over the PR #25 seam (`GET /me/connectable` ∪ `GET /me/projects/:id/connections` → `ConnectionListItem[] { provider, status, accountLabel?, connectedAt, lastSyncAt? }`). Rendered in two groups: **active/connected at the top — green with a tick**, showing the provider, its account label, and last sync; **connectable-but-not-connected, plus disabled, at the bottom — red, in red text.** Selecting a red entry connects it: token-paste (prompt for the provider's own credential → `POST …/connections/:provider {credential}`) where the provider supports an API token, or OAuth (open the browser to the broker's consent URL → the single `/connections/callback`) where it does not. Selecting a green entry offers disconnect / re-consent (`DELETE …/connections/:provider`). Connectors are what fill the daemon's OBSERVED layer, so this is the control surface for half the graph.

2. **Git Repositories** — add repositories and see the ones already on your Hosted NEAT with how current each is. Each repo renders as **green with a tick when synced/up-to-date**, an **in-progress state when it is building or mid-sync from the GitHub bot/app**, or **red with a reason when it is failing to sync** (branch/permission/build context). Adding a repo hands it to the hosted GitHub App to clone + sync, which fills the daemon's EXTRACTED graph. **This section depends on the GitHub App (neat-infra #19), which is not yet deployed.** It consumes a control-plane repos seam this ADR names for #19 to provide: `GET /me/projects/:id/repos → [{ repo, syncStatus: 'synced'|'syncing'|'failed', detail?, lastSyncAt? }]` and a `POST` to add — same `neat_pat_` auth as the rest of `/me`. Until #19 lands, the section renders an honest "repository sync arrives with the GitHub App" placeholder rather than fabricating rows.

3. **Open Web GUI** — opens the profile's web URL (`app.neat.is`) in the browser, reusing the login flow's browser-open. Self-explanatory: the escape hatch to the full console when the terminal view is not enough.

**It rides the daemon connection end to end.** Everything `neat config` changes flows to the hosted daemon the laptop is already connected to: connect a connector → that tenant daemon's OBSERVED layer fills; add a repo → the GitHub App syncs it → the daemon's EXTRACTED graph fills; then the same `neat` CLI / MCP that read the daemon see the richer graph. `neat config` is the control panel for the graph behind the connection `neat login` opened — no separate auth, no separate concept, one connection.

### Consequences

- Terminal-native hosted management: a user who logs in on the terminal never has to leave it to wire up their graph — the "log into Claude on your terminal" feel extends from login to setup. The web GUI remains one keystroke away for the full view.
- **Dependencies, honestly:** the Connectors section is live once PR #25 deploys and the profile carries the `neat_pat_` (the two-credential-profile decision); the Git Repositories section is gated on the GitHub App (#19) and ships when #19 lands (placeholder until then); Open Web GUI works today. So `neat config` lands incrementally — the menu + hosted-gate + web-GUI + connectors first, repositories when #19 is deployed.
