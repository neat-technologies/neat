...
- **ROUND-3 FIX WAVE — MERGED to `main`** — PR **#628** (`5e523e8`), CI green, 5 PRs rolled up: stitcher provenance (#618), watch re-extract (#622), fusion incident reconciliation (#619), daemon OTLP port + `daemon.json` (#621), incident quality (#624). Follow-up **#629** swaps a raw NUL delimiter for ` ` (cosmetic, ready to merge).
- **CAMPAIGN STATE — DECISION POINT.** Across 2 breaker rounds (20 shapes) + 3 fix rounds, **~20 fixes merged to `main` (#606/#613/#628), all CI-green, all verified e2e on real backends.** Every *bounded* bug the breaker found — precision, provenance, fusion, cross-service RCA, blast-radius, daemon, extraction resolution — is fixed and in trunk. ~10 contracts were amended along the way (incl. the blast-radius/ADR-038 supersession + the trace-stitcher allowlist) — worth a governance review pass.
  - **What's left = the OBSERVED-coverage frontier** (#576 inbound/in-process, #595 real call-graph, #614 queue, #615 graphql, #616 grpc, #617 websocket). This is a **build project**, not bug-fixing; designs proposed (#576 ~1 day first-cut, #595 ~1 wk foundational). It's the real road to HN-ready.
  - **Fork for Cem:** (1) greenlight the coverage build (orchestrate #576 first-cut + queue incident-tracking now; hold #595 as its own sprint), or (2) bank the hardening and schedule the coverage frontier as a directed sprint later.
...
- **Coverage code (bounded) → PR #632 merged; `neat.is@0.4.23` PUBLISHED to npm `latest`.** ADR-117 (incidents record from any failure span, not only HTTP status) closes the async/worker incident-visibility gap (#614) — the real gap was OTel's `recordException` leaving span status UNSET. Release PR #633, tag `v0.4.23`, publish workflow green.
- **HELD for a directed sprint (prose-ready, NOT rushed):** the OBSERVED-coverage frontier — #576 inbound/in-process OBSERVED edges, #595 real call-graph extraction, #615 GraphQL, #617 WebSocket. Designs posted on the issues. This is the road to HN-ready; it changes thesis-critical ingest/extraction and warrants a directed sprint rather than tail-of-session autonomy.
- **CAMPAIGN CLOSED.** 2 breaker rounds · ~20 fixes verified e2e · ADRs 110–117 · contracts drift-free · `0.4.22` + `0.4.23` shipped to npm `latest`.
