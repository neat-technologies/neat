...
adr: [ADR-060, ADR-066, ADR-115, ADR-119, ADR-125, ADR-029, ADR-039, ADR-050, ADR-027, ADR-061, ADR-095, ADR-213, ADR-215, ADR-216, ADR-220]
...
| `observed-failing` | A declared dependency production *observes failing* (ADR-220): a declared+observed edge whose observed error rate is over the threshold, or a declared external call whose incident shows a transport/5xx failure fused to the declaring code — declared-and-observed-but-broken, which is neither a missing edge nor a member mismatch | `0.6` (INFERRED join) |
| `deploy-mismatch` | A k8s workload's manifest declares an image the running pods do not report (ADR-225): `declaredImage` (EXTRACTED, from `extract/infra/k8s.ts`) ≠ `observedImage` (OBSERVED, stamped by the k8s reader) on one `service:<name>` node — the *stuck rollout* where the deploy did not take, the service is up on the old image, and no incident fires. `kind: 'image'` today; `kind: 'replicas'` is reserved | `0.9` (both sides exact facts) |

The five edge-grain types were the original locked set; `observed-symbol-mismatch` is the sixth, added at symbol/field grain by its successor ADR-215 (§5f); `observed-failing` is the seventh, added by ADR-220 (§5g) for a declared dependency observed failing; `deploy-mismatch` is the eighth, added by ADR-225 for the k8s deployment substrate — a node-local declared-vs-observed image compare on the service node, the stuck rollout an outage reader can't see. The taxonomy does not grow a new type for schema-grain drift (ADR-157 §4). `missing-observed` and `missing-extracted` gain a **column-grain locus** alongside their edge locus: for a `sql-table` node carrying both declared and observed columns, a declared-only column is `missing-observed` and an observed-only column is `missing-extracted`, reported at column grain (`orders.total`) through the same `get_divergences`. A column present on both sides is fused, not drift; a node with only one side present emits nothing — no drift claim without both a declared and an observed column on the table (the ADR-141 fusion discipline). This is the same semantics the edge-triple detectors run, computed over the declared and observed column sets on one node — so every surface that already renders these two reasons renders column drift with no new type.
...
### 5g. Behavioral-failure divergence (ADR-220)

The five edge-triple types ask *"does a declared edge have an observed twin?"* and the symbol type (§5f) asks *"does the runtime shape match the declared access?"*. Neither reaches the case where a dependency the code declares **and** production observes has calls that **predominantly fail**: the edge is present (so not `missing-observed`, not `missing-extracted`), the access is fine (so not `observed-symbol-mismatch`), yet "the code declares this dependency and in production it fails every call" is the truest declared-intent-vs-observed-reality divergence there is. `observed-failing` is the seventh type. It surfaces at two loci and, like §5f, is graded INFERRED (`0.6`) — the cross-layer join between the declared call and the observed failure.

- **Edge locus.** A declared+observed dependency edge whose observed error rate is over the threshold. Both provenances must be present: an observed-only failing edge is already the headline `missing-extracted` finding (and has no declared intent to diverge against), and a declared-only edge has no observed calls to fail — so neither is re-flagged. "Predominantly failing" is `errorCount / spanCount >= 0.5` over an edge with `spanCount >= 5`. Half is the defensible line between *degraded* (a working dependency erroring on a minority of calls — an SLO/alerting concern, not a divergence) and *broken* (the declared intent is not realized for most calls); the 5-span floor keeps a one-off or low-volume blip off the surface. The `spanCount`/`errorCount`/`errorRate` ride on the finding as evidence. This holds the existing false-positive discipline: a healthy or flaky-but-working edge never trips 50%.

- **Incident locus.** A declared external call whose recorded incident indicates a transport/5xx failure, fused to the declaring `file:line` through the same locus machinery §5f uses (`symbolLocus`, ADR-215; recoverable from a stacktrace by ADR-216). This covers the wrong-host / connection-refused and deadline cases where the failing call mints no successful observed edge, so the signal lives only in the incident store — read from `opts.incidents` (§4), never re-fetched, so the function stays pure. Repeated incidents of one `(locus, family)` collapse to a single finding carrying the `incidentCount`.

**Classified on generic failure *semantics*, never a provider/framework/host/language name** (the ADR-158 §6 invariant, scanned). The failure kinds are a closed, neutral set: `error-rate` (the edge locus), and the incident-locus transport families `connection-refused`, `deadline-exceeded`, `timeout`, `unavailable`, `server-error`. The transport families match cross-language semantics from the error text — a gRPC/status name (`DEADLINE_EXCEEDED`, `UNAVAILABLE`), a POSIX errno (`ECONNREFUSED`, `ETIMEDOUT`, `ECONNRESET`), and their plain-English equivalents — so a new phrasing joins a family and a new language never gets a branch; a 5xx is classified structurally off the incident's `httpStatusCode` (500–599), never a message substring, and a 4xx is a client-side concern left out.

**Ranked below the definitive structural and symbol divergences.** At `0.6` it sorts beneath `version-mismatch`/`compat-violation` (1.0) and structural `missing-extracted`, and above a dampened dead-code probe (§5e, 0.1); the type-leadership tiebreak places it last, so at equal confidence a structural or symbol finding leads. The edge and incident loci are complementary readings — the failing edge as the caller sees it, and the declaring call site with the runtime's own reason — and each de-duplicates within itself. Schema growth by successor ADR, not a re-reading of the six; nothing on the existing types changes.
