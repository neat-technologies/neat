...
adr: [ADR-037, ADR-114, ADR-014, ADR-029, ADR-031, ADR-158, ADR-189, ADR-190, ADR-191, ADR-209, ADR-214, ADR-220, ADR-222, ADR-223, ADR-226]
...
Because the culprit is unobservable, the cause is not named from a settled cause but **read back from a staged FRONTIER surface** (ADR-226). A hang culprit walked from declared topology is a *guess about a thing NEAT cannot see*, so it must not be laundered as an INFERRED settled stitch. Instead the **hang sensor** stages a FRONTIER edge on the unobservable hop (a post-ingest analysis pass; route-driven, crossing one observed hop to reach a culprit behind an infra proxy), and `getRootCause` reads that surface: the settled verdict **leads** — the boundary is `candidates[0]` and the `rootCauseNode`, classified `symptom-only` — and **below** it, when a surface is staged, the proposed culprit is surfaced at **FRONTIER** provenance, a FRONTIER-ceiling confidence (0.2, below STALE), ranked beneath every settled cause, its reason stating it is proposed from a staged surface, graduates to OBSERVED when the service recovers, and is a hypothesis to confirm or cull. `getRootCause` **reads** the surface (a `FRONTIER` edge in the graph); it stages nothing and traverses no FRONTIER edge — the sensor is the only writer. When no surface is staged — the sensor hasn't seen the hang, an infra boundary with no declared serving path, or a route that can't be narrowed — the verdict stays **honest-coarse**: the boundary is `symptom-only` and *no* cause is proposed, the reason saying the cause is downstream and unobserved. Never a fabricated callee, never a guess dressed as a settled cause — the coverage bound is inherent (a span that never exports cannot be observed), a recall bound made honest. Full rationale: [ADR-222](../decisions.md#adr-222--a-boundary-timeout-is-a-symptom-not-the-fault-classify-it-symptom-only-and-point-structurally-upstream-when-the-culprit-is-unobservable), revised by [ADR-226](../decisions.md#adr-226--frontier-is-the-graphs-edge-a-promotable-surface-for-what-sits-outside-the-settled-graph-kernel-step-1-the-hang-as-first-user).
