...
§1–§9 describe connectors that observe *traffic* and fuse it onto an edge. A second, narrower kind observes a **failure event** the provider already recorded and maps it onto the repo node responsible, so an agent can query `get_incident_history` / `get_root_cause` over it and fix the cause in the repo. **EAS is the first** (ADR-185): it pulls `ERRORED` Expo/EAS builds from the Expo GraphQL API and mints one OBSERVED build-failure incident per build.

The incident-emitting **shape** is reused beyond the vendor plane. The **Kubernetes deployment substrate** (#1124, ADR-224) uses this exact `ConnectorIncident` shape and the same `runConnectorPoll` write path to mint an OBSERVED incident when a workload is down for a *deployment* reason — a bad image (`ImagePullBackOff`), zero replicas (`spec.replicas: 0`), a crashloop (`CrashLoopBackOff`), the faults a dead pod emits no span for. But Kubernetes is **not** a `neat connector` vendor (it is ubiquitous infra with a declared/observed divergence story, not a SaaS NEAT pulls from): its observed cluster-state reader reuses this poll/incident plumbing while being enabled off a separate substrate surface (`docs/connectors/kubernetes.md`, ADR-224), not `PROVIDER_DISPATCH`. Its dedupe id is stable per `(namespace, deployment, fault)` — a current-state fact — rather than EAS's per-build id.
