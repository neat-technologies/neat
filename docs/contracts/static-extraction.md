...
adr: [ADR-032, ADR-065, ADR-115, ADR-119, ADR-123, ADR-030, ADR-031, ADR-024, ADR-055, ADR-133, ADR-138, ADR-155, ADR-158, ADR-161, ADR-192, ADR-193, ADR-194]
...
Go services are discovered from `go.mod`. `tree-sitter-go@0.21.2` declares `tree-sitter ^0.21.0`, matching the repository's native binding without an ABI upgrade (ADR-154). Local package imports resolve only when one non-test source file is unambiguous; Gin, Echo, Fiber, and Chi routes require literal method paths and literal in-file prefixes — a `Group("/prefix")` variable for Gin/Echo/Fiber, a `Route("/prefix", func…)` closure stack for Chi — composed across nesting; net/http (Go 1.22+ ServeMux) routes are gated structurally on the `net/http` import plus the method-prefixed pattern shape rather than a `go.mod` dependency, since the package is stdlib; raw-SQL `database/sql` / `sqlx` calls require a literal statement resolving to one table and are gated on the driver import (ADR-184). Ambiguous or computed identities stay unattributed. Go also reads at table grain: `calls/gorm.ts` (ADR-180) is the GORM data axis, gated per-file on the `gorm.io/gorm` import. Unlike the Rails/Laravel readers, GORM has **no literal schema file** — the table name is derived from the struct name by GORM's own `inflection.Plural(toDBName(name))`, so the derivation is the fusion key and a wrong plural mints a false node, not a missed edge (the OBSERVED table, `db.collection.name` per ADR-179, is the ground-truth anchor). Go reaches symbol grain: `symbols.ts` mints a `SymbolNode` per `func` and receiver `method` (`Receiver.method`), ADR-192. File-grain call-site stamping — the `code.*` anchor an observed Go span carries to fuse onto those symbols — is the installer's job, a separate rung, and standard Go auto-instrumentation does not emit it.

Ruby services are discovered from the `Gemfile` (`extract/ruby.ts`), the analog of the `go.mod` reader — a Gemfile names no project, so the service takes its directory's basename and each `gem` line is a dependency gate. `tree-sitter-ruby` is pinned at exactly `0.21.0`, the one ABI-14 release: the grammar jumps to ABI-15 at `0.23.0`, which the pinned `tree-sitter@^0.21.1` runtime can't load, so the pin is exact, not a caret (ADR-173). Ruby reaches route, service, and symbol grain: `.rb` files are FileNodes, `config/routes.rb` is read for Rails routes, and `symbols.ts` mints a `SymbolNode` per `method` / `singleton_method` (`Class.method`, an instance `initialize` the constructor) and `class` / `module` (ADR-193) — Ruby's `Class#method` and `::` namespace both reduce to the dotted qualname the JS/TS and Python walkers produce, so a runtime `code.function` fuses onto it by the same span-containment key. File-grain call-site stamping — the `code.*` anchor an observed Ruby span carries to fuse onto those symbols — is the installer's job, a separate rung, and standard Ruby auto-instrumentation does not emit it. Ruby's autoload-driven require graph stays unmodelled — `imports.ts` skips `.rb` rather than parse it with the wrong grammar.
...
| `calls/gorm.ts` | `infra:sql-table:<name>` + EXTRACTED `columns` from GORM model structs (`.go`), the table name DERIVED by GORM's `inflection.Plural(toDBName(name))` since GORM has no literal schema, plus belongs-to/has-one/has-many/many2many `REFERENCES` edges (ADR-180) | ✅ |
| `calls/go.ts` | `infra:sql-table:<name>` + EXTRACTED `columns` from raw-SQL `database/sql` / `sqlx` call sites (`.go`), the table and columns recovered by the same `tableFromSqlStatement` / `columnsFromSqlStatement` the OBSERVED ingest runs on `db.statement` / `db.query.text`, gated on the driver import (ADR-184) | ✅ |
| `calls/go.ts` | `infra:sql-table:<name>` + EXTRACTED `columns` from raw `database/sql` / `sqlx` call sites (`.go`), gated on the driver import; table and columns parsed by the same `tableFromSqlStatement`/`columnsFromSqlStatement` helpers the OBSERVED side runs on `db.statement`/`db.query.text`, so the two fuse (ADR-184) | ✅ |
| `table-edges.ts` | `infra:sql-table:<child> ──REFERENCES──▶ infra:sql-table:<parent>` foreign keys read per-ORM (Drizzle `.references`, Prisma `@relation(fields:)`, SQLAlchemy `ForeignKey`, Rails/Laravel/GORM associations), parent at DB-name fidelity (ADR-161) | ✅ |
...
`calls/gorm.ts` (ADR-180) is the Go data axis, the model reader for GORM the way `calls/activerecord.ts` is for Rails and `calls/eloquent.ts` is for Laravel. Gated per-file on the `gorm.io/gorm` import, it parses `.go` via `tree-sitter-go` (the raw file, not the JS-comment-masked copy, so Go `//` comments are excluded structurally as `comment` nodes). The load-bearing difference from Rails/Laravel: **GORM has no literal schema file** — there is no `schema.rb`/migration whose names are the exact database strings. The table name is DERIVED from the struct name by GORM's `schema.NamingStrategy.TableName` = `inflection.Plural(toDBName(name))`, so the derivation IS the node identity and must reproduce GORM byte-for-byte: `toDBName` is the initialism-aware CamelCase→snake port (the golint `commonInitialisms` replaced Title-case first, so `APIKey` → `api_key` not `a_p_i_key`, `LineItem` → `line_item`), and `Plural` reproduces `github.com/jinzhu/inflection` — uncountables (`series`, `equipment`), irregulars (`person`→`people`, `child`→`children`), and the ordered rules (`-y`→`-ies`, `-s/-x/-ch/-sh`→`-es`). A wrong derivation mints a table node the runtime never creates (a **false** node), so the OBSERVED table (`db.collection.name`, ADR-179, GORM's own resolved `tx.Statement.Table`) is the ground-truth anchor and the finite irregular set is a documented best-effort. `gormEndpointsFromFile` reads each model struct — one that embeds `gorm.Model`, is passed to `db.AutoMigrate`/`Model`/`Create`/`Find`/`First`/`Where`, has a `TableName()` method, or is referenced as a relation by another model — into an `infra:sql-table:<name>` node carrying its columns as `ColumnAttr`s (ADR-157 §3): each scalar field snake_cased (or the `gorm:"column:x"` override verbatim), `gorm.Model` expanded to `id`/`created_at`/`updated_at`/`deleted_at`, an anonymous embedded local struct inlined, `gorm:"-"` skipped, and a struct/slice-of-struct field excluded as a relation (its `<Field>ID` companion scalar stays a column). `gormForeignKeys` mints `REFERENCES` edges (ADR-161): belongs-to (`User` holds `CompanyID` + `Company Company`) points this table → the association; has-one/has-many put the FK on the other table; `many2many:` synthesizes the join table with an edge to each side. A `TableName()` literal override is honored; `db.Table(...)` per-call and a global `NamingStrategy{TablePrefix, SingularTable}` fall back to the derived name (best-effort). Cross-file type resolution beyond `AutoMigrate`, cross-package relations, and file-grain are deferred; the OBSERVED twin fuses on the table name the same way every other ORM NEAT reads does.

`calls/go.ts` (ADR-184) is the **raw-SQL** Go data axis — the reader for services that skip the ORM and hand `database/sql` or `github.com/jmoiron/sqlx` a literal statement, beside `calls/gorm.ts`'s ORM path. ADR-154 seeded it with the six base `database/sql` query/exec methods and the table alone; this rounds it out into a gated, columns-aware, sqlx-covering reader. Gated per-file on the `database/sql` and/or `github.com/jmoiron/sqlx` import — the structural gate the net/http route recognizer (ADR-182) uses, so an arbitrary Go string never mints a table — it reads the full statement-taking surface: `database/sql`'s `Query`/`QueryContext`/`QueryRow`/`QueryRowContext`/`Exec`/`ExecContext`/`Prepare`/`PrepareContext`, plus sqlx's `Get`/`Select`/`Queryx`/`QueryRowx`/`NamedExec`/`NamedQuery`/`MustExec`/`Preparex`/`GetContext`/`SelectContext` (the sqlx-only names are recognized only under the sqlx import — they are common method names — while the base methods are read under either import, since a `*sqlx.DB` embeds `*sql.DB`). The SQL is the **first string-literal argument**, not a fixed position: that skips a leading `ctx` on the `*Context` calls and the destination pointer sqlx's `Get(&u, "…")` / `Select(&xs, "…")` take, and reads both the interpreted `"…"` and the raw backtick `` `…` `` form (multi-line backtick SQL is idiomatic in Go); a statement that isn't a static literal — `fmt.Sprintf(…)`, a `+` concatenation — is a call/binary expression, not a string literal, so it is left unclaimed rather than guessed. Each statement runs through the **same** `tableFromSqlStatement` → table and `columnsFromSqlStatement` → columns helpers the OBSERVED ingest runs on `db.statement` / `db.query.text` (ADR-152), so the extracted table name is byte-identical to the observed one and the two fuse on `infra:sql-table:<name>`; the touched columns fold on as `ColumnAttr`s (ADR-157 §3) through the same `foldColumns` path the ORM readers use. A statement whose table doesn't resolve (a JOIN, a multi-`FROM` subquery) is skipped, never guessed; cross-file prepared-statement handles, query-builder chains (squirrel, goqu), and runtime-assembled statements are deferred.

`calls/go.ts` (`goSqlEndpointsFromFile`, ADR-184) is the raw-SQL reader beside the GORM one — the columns-aware, import-gated successor to ADR-154's minimal `database/sql` seed, for the Go services that hand a driver a literal SQL string instead of using an ORM. Gated per-file on a `database/sql` and/or `github.com/jmoiron/sqlx` import (the structural gate net/http routes use, ADR-182), it parses `.go` via `tree-sitter-go` and matches statement-taking call sites: `database/sql`'s `Query`/`QueryContext`/`QueryRow`/`QueryRowContext`/`Exec`/`ExecContext`/`Prepare`/`PrepareContext`, plus sqlx's `Get`/`Select`/`Queryx`/`QueryRowx`/`NamedExec`/`NamedQuery`/`MustExec`/`Preparex`/`GetContext`/`SelectContext` (the sqlx-only methods recognized only when sqlx is imported; the base methods under either, since `*sqlx.DB` embeds `*sql.DB`). The SQL is the **first string-literal argument**, not a fixed position — that skips a leading `ctx` on the `*Context` variants and the destination pointer sqlx's `Get(&u, "…")` takes, and reads both the interpreted `"…"` and the raw backtick `` `…` `` form (multi-line SQL). A non-literal statement (`fmt.Sprintf(…)`, a concatenation, a query-builder chain) is left **unclaimed** rather than guessed. Each resolved statement runs through the OBSERVED SQL-parse helpers **verbatim** — `tableFromSqlStatement` for the `infra:sql-table:<name>` node and `columnsFromSqlStatement` for its columns as `ColumnAttr`s (ADR-157 §3), folded through the same `foldColumns` path as the ORM readers — so the extracted table name is byte-identical to the one ingest recovers from `db.statement` / `db.query.text` (ADR-152, there being no dedicated table attribute) and the two fuse rather than twinning. That reuse is the fusion key and the whole reason the reader is worth more than a static table list. A statement whose table doesn't resolve (a JOIN, a multi-`FROM` subquery) is skipped; one endpoint is emitted per call site so a SELECT and an INSERT to the same table each fold their columns onto the one node. It grades `verified-call-site` (0.85), clearing the precision floor. Cross-file prepared-statement handles and runtime-assembled SQL are deferred.
...
| Gin / Echo / Fiber / Chi (Go) | `<router>.VERB("/path/:id", h)` read with `tree-sitter-go`, gated on the framework's `go.mod` require — `github.com/gin-gonic/gin`, `github.com/labstack/echo(/v4)`, `github.com/gofiber/fiber/v2`\|`/v3`, or `github.com/go-chi/chi(/v5)` (Gin per ADR-154, Echo + Fiber per ADR-181, Chi per ADR-182). All four share the verb read: a router value's HTTP-verb method (Gin/Echo upper-case `GET`, Fiber/Chi title-case `Get`) whose first string argument is the path. Gin/Echo/Fiber track `<router>.Group("/prefix")` to its variable so the prefix composes onto every route registered on it — across nested groups, so `v1 := admin.Group("/v1")` reaches `/admin/v1`. **Chi composes differently**: `Route("/articles", func(r chi.Router){ … })` has no assignment — the prefix is lexically scoped to the closure body and the inner `r` shadows the outer — so its recognizer carries a **prefix stack** (push the literal on the `func_literal` body, pop on return, nested `Route`s composing `/articles/{articleID}`) rather than a variable map; Chi's `Group(func(r chi.Router){ … })` (first argument a closure, not a string) is middleware grouping and contributes no prefix; and Chi's `Mount("/admin", h)` is **deferred** — the mounted sub-router is built in another func/file (the cross-file resolution Express `app.use` needed before ADR-160), so it composes no route, which costs one fused edge since otelchi emits the full composed path. The template keeps the `:id` / `:id?` / `*` / `{id}` form verbatim (Fiber's optional-param `?` drops at canonicalization, the Laravel `{id?}` precedent; Chi's inline regex `{id:[0-9]+}` strips to `{id}`) — `otelecho` (`c.Path()`) and `otelfiber` / the Fiber v3 middleware (`c.Route().Path`) both set `http.route` to exactly the group-composed template, and **`otelchi` (`github.com/riandyrn/otelchi`) does the same from `chi.RouteContext(r).RoutePattern()`** — so it fuses through `normalizePathTemplate` (which collapses `:id`/`:id?`/`{id}` to `:param` and leaves a bare `*` literal on both sides) with no ingest change. **Chi's fusion is conditional on otelchi**: under bare `otelhttp` a Chi app sets no `http.route` (Chi never populates `r.Pattern`), so those RouteNodes stay EXTRACTED-only. Route-param constraints (Echo/Fiber's typed `:id` syntax), `Static` / `Use`-middleware-only mounts, Chi's generic `Method`/`MethodFunc` and cross-file `Mount`, and routes registered dynamically in loops are deferred — those surface as observed-but-not-declared divergence rather than a fabricated node. |
| net/http (Go 1.22+) | Go 1.22's `ServeMux` carries method + template in one string literal — `mux.HandleFunc("GET /orders/{id}", h)`, `http.HandleFunc(…)`, `mux.Handle(…)`, `http.Handle(…)` — read with `tree-sitter-go` (ADR-182). The recognizer reads the first string-literal argument, splits on the first space, and emits a RouteNode **only when the leading token is an HTTP method AND the remainder starts with `/`**. Because net/http is stdlib there's **no `go.mod` dependency to gate on**, so the gate is structural: the file must `import "net/http"`, and only the method-prefixed pattern shape mints a route. That precision is what keeps a generic `HandleFunc("literal", h)` — or the pre-1.22 bare-prefix `HandleFunc("/orders/", h)` (no method, coarse subtree, the biggest false-positive source) — from being read; the bare-prefix form is deliberately deferred. `{id}` (single segment), `{path...}` (multi-segment wildcard), and `{$}` (end-of-path anchor) all collapse to `:param` under `normalizePathTemplate` with no special casing. **Auto-fusion is version-gated**: on a modern stack (Go 1.23 `Request.Pattern` + otelhttp ≥ v1.36) the instrumentation sets `http.route` to the path-only template (method stripped), exactly what's stored, so it fuses with no ingest change; on an older stack `http.route` is absent and the node stays EXTRACTED-only — an honest gate, documented not guessed. Host-prefixed patterns (`"GET example.com/orders"`) and the generic `mux.Method(...)` helpers are out of scope. |
...
Static extraction reaches symbol grain under the file. `symbols.ts` parses each source file with the grammar its extension selects (`SYMBOL_GRAMMAR_BY_EXT` — the JS/TS `GRAMMAR_BY_EXT` extended with `.py` → `tree-sitter-python` and `.go` → `tree-sitter-go`, ADR-192, and `.rb` → `tree-sitter-ruby`, ADR-193) and mints one `SymbolNode` per function, method, constructor, and class **definition**: the JS/TS `const foo = () => {}` arrow/function-expression form, Python `def` / `class` (a decorated `def` unwrapped, `__init__` the constructor), Go `func` and receiver `method` (`Receiver.method`; Go has no class), and Ruby `method` / `singleton_method` (`Class.method`, an instance `initialize` the constructor) and `class` / `module` (a `::` scope-resolution name normalized to the dotted qualname). A per-language walker reads each language's definition node types into the one shape, and `.py`/`.go`/`.rb` live in `symbols.ts`'s own map so the sibling AST producers that import `GRAMMAR_BY_EXT` (`symbol-edges.ts`, `actions.ts`, `zod-shapes.ts`, `calls/drizzle.ts`, `calls/firestore.ts`) stay JS/TS-scoped. Each symbol carries its source-declared `qualname` (`OrderService.create`, `merge`), its `kind`, and its definition span `{ startLine, endLine }`, and is owned by its file through a `file ──CONTAINS──▶ symbol` edge — the same containment shape files use under services (file-awareness.md §2), one level deeper. The edge is `structural`-graded EXTRACTED with `evidence.file`/`line` pinned to the definition, and every write is `hasNode` / `hasEdge`-guarded like every other producer.

The node id is `symbolId(service, relPath, qualname, disambiguator?)` → `symbol:<service>:<relPath>#<qualname>`, built from the identity helper ([identity.md](./identity.md)); same-named siblings in one file get an ordinal `~<n>` in source order so the id stays collision-free without inventing a name. The node is language-neutral — the tree-sitter grammar is the per-language adapter — so a symbol produced from JS/TS, Python, Go, or Ruby is the same shape (ADR-192, ADR-193). The span is the fusion key ingest joins a runtime `code.line` against to land an OBSERVED edge on the calling symbol ([otel-ingest.md](./otel-ingest.md) / file-awareness.md §4). An observed call landing where this producer emitted no symbol mints a `discoveredVia:'otel'` twin in ingest (lifecycle.md), which is the `missing-extracted` signal at symbol grain — static-first fields override it on the next pass.
