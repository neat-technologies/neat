...
| Gin / Echo / Fiber (Go) | `<router>.VERB("/path/:id", h)` read with `tree-sitter-go`, gated on the framework's `go.mod` require — `github.com/gin-gonic/gin`, `github.com/labstack/echo(/v4)`, or `github.com/gofiber/fiber/v2`\|`/v3` (Gin per ADR-154, Echo + Fiber per ADR-181). The three share one recognizer: a router value's HTTP-verb method (Gin/Echo upper-case `GET`, Fiber title-case `Get`) whose first string argument is the path, with `<router>.Group("/prefix")` tracked to its variable so the prefix composes onto every route registered on it — across nested groups, so `v1 := admin.Group("/v1")` reaches `/admin/v1`. The template keeps the `:id` / `:id?` / `*` form verbatim (Fiber's optional-param `?` drops at canonicalization, the Laravel `{id?}` precedent) — `otelecho` (`c.Path()`) and `otelfiber` / the Fiber v3 middleware (`c.Route().Path`) both set `http.route` to exactly this group-composed template, so it fuses through `normalizePathTemplate` (which collapses `:id`/`:id?` to `:param` and leaves a bare `*` literal on both sides) with no ingest change. Route-param constraints (Echo/Fiber's typed `:id` syntax), `Static` / `Use`-middleware-only mounts, and routes registered dynamically in loops are deferred — those surface as observed-but-not-declared divergence rather than a fabricated node. |
