...
`calls/sqlalchemy.ts` is the Python declared-column producer, extending the ADR-152 table read: for a declarative model it reads each `x = Column(...)` / `x: Mapped[…] = mapped_column(...)` assignment in the class body, where the DB column name is the explicit first-string argument if present (`Column('total_amount', Integer)` → `total_amount`) and the **attribute name itself** otherwise (`user_id = Column(Integer)` → `user_id`, SQLAlchemy's own rule) — a `relationship(...)` or non-`Column` assignment is not a column. Raw `CREATE TABLE`, Prisma model fields, and TypeORM are named producer follow-ons, each a new recognizer emitting `columns` on the endpoint, not a model change.
