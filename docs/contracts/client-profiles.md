...
| `neat sync --to <url\|profile>` | **deliberate remote write** | The one write that crosses to a remote daemon: pushes a freshly extracted snapshot (ADR-074). `--to` may name a profile. It resolves the target project from the daemon rather than the local directory name — a hosted tenant daemon hosts exactly one project (`GET /projects` marks it `hostedHere: true`), and the snapshot lands under that name, so the local repo need not be named the same as the hosted project. When `GET /projects` is unreachable or reports no `hostedHere` entry it falls back to the local project name, keeping local/self-host sync unchanged. |
