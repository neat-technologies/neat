...
| `GET /graph/incident-card/:nodeId` | the composed incident work order (ADR-221): the incident fused with its root-cause chain, blast radius, applicable policies, and node divergence, each claim provenance-stamped. `?errorId=` pins one incident; default is the node's most-recent. Backs `get_incident_card` and the monitor `incident` fact | `IncidentCard` |
