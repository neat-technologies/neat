...
**Java, Ruby, .NET, Go, Rust** — out of MVP. Each requires a successor ADR. Python reaches apply-side parity with the Node generated-file pattern through the `neat_otel.py` call-site processor (ADR-151): the entry point is resolved from the Procfile run command (`uvicorn`/`gunicorn`/`python`/`fastapi run`) or the conventional entry-point names, the processor is written and its import injected idempotently, and the SERVER span stays route/service-grained (it is created before the handler runs) while CLIENT/PRODUCER DB and HTTP spans reach file grain.
