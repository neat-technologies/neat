...
| Python | `opentelemetry-distro`, `opentelemetry-exporter-otlp` → `requirements.txt` or `pyproject.toml` | `neat_otel.py` — the call-site span processor (ADR-151), stamping `code.file.path` / `code.line.number` / `code.function.name` on CLIENT/PRODUCER spans; degrades to a no-op when OpenTelemetry is absent | `import neat_otel` injected at the resolved entry point (after a shebang / `from __future__`), plus the `opentelemetry-instrument` prefix on the Procfile command | `OTEL_EXPORTER_OTLP_ENDPOINT` set in orchestration layer |
| Go | OTel API, SDK, and OTLP/HTTP exporter requirements → `go.mod`; `go.sum` is never written | `neat_otel.go` beside the resolved `main.go`; `sdktrace.SpanProcessor.OnStart` + `runtime.Callers` stamps stable `code.*` attributes | None: the same-package generated file participates in compilation and registers through `init()` | Standard OTel env vars; optional `NEAT_SERVICE_ROOT` override |
| Ruby | `opentelemetry-sdk`, `opentelemetry-exporter-otlp`, `opentelemetry-instrumentation-all` → `Gemfile`; `Gemfile.lock` is never written (ADR-186) | `config/initializers/neat_otel.rb` for a Rails app — `OpenTelemetry::SDK.configure` with `use_all`, plus a `caller_locations` span processor stamping `code.file.path` / `code.line.number` / `code.function.name` on CLIENT/PRODUCER spans; degrades to a no-op when the gems are absent | None: Rails convention-loads `config/initializers/*` at boot | Project-scoped OTLP path baked in the initializer, overridable via the standard OTel env vars |
| PHP | `open-telemetry/sdk`, `open-telemetry/exporter-otlp`, `php-http/guzzle7-adapter`, and `open-telemetry/opentelemetry-auto-laravel` (Laravel) → `composer.json` `require`; `composer.lock` is never written (ADR-186) | `neat_otel.php` bootstrap — points the exporter at the project-scoped path and turns on `OTEL_PHP_AUTOLOAD_ENABLED`; degrades to a no-op when the `opentelemetry` extension is absent | None: wired via `auto_prepend_file` or a top-of-`index.php` require | Project-scoped OTLP path + service name baked in the bootstrap. **Requires the `opentelemetry` PECL extension** — a system install composer cannot do; the plan surfaces it as a required user step. File grain deferred (ADR-186) |

**Java, .NET, Rust** — out of MVP. Each requires a successor ADR. Ruby (Rails) and PHP (Laravel) joined the in-scope set in ADR-186 — Ruby reaching file grain through its call-site processor, PHP landing route/table/service grain with the PECL requirement surfaced and file grain a documented follow-up. Python reaches apply-side parity with the Node generated-file pattern through the `neat_otel.py` call-site processor (ADR-151): the entry point is resolved from the Procfile run command (`uvicorn`/`gunicorn`/`python`/`fastapi run`) or the conventional entry-point names, the processor is written and its import injected idempotently, and the SERVER span stays route/service-grained (it is created before the handler runs) while CLIENT/PRODUCER DB and HTTP spans reach file grain.
...
**The post-apply install is language-aware (ADR-186).** That automatic package-manager run fires only for a `package.json` edit. The orchestrator keys on the manifest basename of the dependency edits: `package.json` → the JS package manager above; `Gemfile` → `bundle install`; `composer.json` → `composer install`; `go.mod` → `go mod download`. For the non-JS manifests NEAT does **not** run the command — it can't guarantee bundler / composer / the Go toolchain is on PATH, and defaulting to `npm install` there would walk up to an unrelated ancestor `package.json` and resolve the wrong dependencies while the real ones stayed uninstalled. Instead NEAT stages the manifest edit and surfaces the native command in the apply summary (the `dependencyInstructions` tally) for the operator to run — the same instruct-don't-execute pattern the PHP PECL step uses. Only the JavaScript case spawns a process.
