...
A status-only gate was blind to the two attribute forms — the dominant failure representation on gRPC/HTTP microservice stacks. On an otel-demo `checkout` whose 3,349 failing spans were all `rpc.grpc.status_code=13` (INTERNAL) with UNSET status, every edge read `errorCount: 0` and `get_root_cause` found no failing chain to walk, so RCA had nothing to root on. Reading the attributes closes that (`spanRecordsError`, ADR-210).
