---
name: otel-ingest
description: OTel receiver replies before mutation, lastObserved derives from span time, parent-span cache correlates cross-service CALLS, exception data is parsed from span events, unseen services and DBs are auto-created, queue producers and consumers mint file-grained messaging edges to the destination topic, GraphQL execution spans mint an operation-grain CONTAINS edge, gRPC execution spans mint a method-grain CONTAINS edge, WebSocket upgrade spans mint a channel-grain CONNECTS_TO edge, span-derived edges always carry OBSERVED provenance, the same edge-minting primitives serve pull-based connector signals, a sibling /v1/logs receiver feeds the logs surface without touching the graph, the HTTP receiver decompresses a gzip- or deflate-encoded body before parsing so a standard OTel Collector (gzip-by-default) ingests, an unroutable bare /v1/traces batch replies 200 with a populated partialSuccess (rejected count + error message) rather than an empty one that reads as full acceptance.
governs:
  - "packages/core/src/ingest.ts"
  - "packages/core/src/otel.ts"
  - "packages/core/src/otel-grpc.ts"
  - "packages/core/src/otel-logs.ts"
  - "packages/core/src/stacktrace.ts"
adr: [ADR-033, ADR-113, ADR-117, ADR-118, ADR-121, ADR-122, ADR-123, ADR-124, ADR-125, ADR-132, ADR-029, ADR-030, ADR-068, ADR-152, ADR-157, ADR-179, ADR-190, ADR-191, ADR-208, ADR-210, ADR-211, ADR-216]
...
A SQL span carries the *table* it operated on, one grain finer than the `database:<host>` node the `CONNECTS_TO` edge lands on — but, unlike the mongoose case, the SQLAlchemy and raw dbapi/psycopg instrumentations emit **no table attribute**. for those instrumentations `db.sql.table` and `db.collection.name` are never set and the table lives only in the `db.statement` SQL text (verified against a live instrumented app — ADR-152). Newer ORM instrumentations do emit the table directly — the current GORM plugin sets `db.collection.name` on a relational system (new semconv) and older ones set `db.sql.table` — and the SQL as `db.query.text` rather than `db.statement`; `parseOtlpRequest` reads the direct table attribute when present (the ORM's own resolved name, more reliable than parsing) and reads either SQL key otherwise, so an ORM on the newer semconv fuses too (ADR-179). Absent a direct attribute, `parseOtlpRequest` recovers the table from the SQL text with `tableFromSqlStatement`: the single identifier after `FROM` / `INTO` / `UPDATE`, quote- and schema-stripped, degrading to `undefined` on a joined or multi-`FROM` (subquery) statement rather than guessing. When a table resolves, the span mints an OBSERVED `CALLS` edge to `infra:sql-table:<name>` — the engine-agnostic table node `extract/calls/sqlalchemy.ts` (ADR-151) also produces from the model's declared/derived table — so the declared and observed table access fuse onto one node rather than twinning. The engine stays on the `database:<host>` node one layer up (ADR-141); the table edge is additive, never a replacement for the database-grain `CONNECTS_TO`. It is file-grained through the same call-site path as any other OBSERVED edge, and service-level otherwise, honestly.

## SQL spans annotate the columns they touch, one grain below the table (ADR-157, refs #918)

The same `db.statement` that names the table also names the columns the statement touched — production statements are parameterized (`INSERT INTO orders (amount) VALUES ($1)`: values redacted, column names present), so the column list is in the text even when the values are not. `parseOtlpRequest` recovers it with `columnsFromSqlStatement` alongside the table: the INSERT column list, the UPDATE `SET` targets, the SELECT projection, and the WHERE predicate columns of each — quote-, schema-, and qualifier-stripped, the SQLAlchemy `<table>.<col> AS <table>_<col>` alias dropped to recover the **real** column (`orders.id AS orders_id` → `id`, never `orders_id`), aggregate / function / `*` projections skipped. It degrades to no columns on the same JOIN / multi-`FROM` (subquery) shapes `tableFromSqlStatement` degrades to `null` on — there the columns belong to more than one table and are never attributed to one.

`handleSpan` merges the recovered columns onto the `sql-table` node it just minted the edge to, as **provenanced attributes on that node — never their own nodes** (ADR-157 §1): an unseen column is added with `OBSERVED` in its provenance set, and a column already present (an `EXTRACTED` schema declaration, or an earlier `OBSERVED`) gains `OBSERVED` in that set while **keeping any existing `EXTRACTED`** — production touching a declared column records both sides on one entry, it does not overwrite the declaration — and a name is never duplicated. The merge is the `mergeObservedColumns` primitive in `ingest.ts` — mutation stays with the lifecycle authority (ADR-030). It is additive and honest: a statement that recovers no columns (`SELECT *`, a join/subquery degrade, an aggregate) grows nothing, and the table edge above stands whether or not any column resolved. This gives "which columns does production read and write on this table" from the statement NEAT already parses, with no declared side required.
...
An inbound HTTP `SERVER` span names the route it served in `http.route` (the templated path the router matched, `/users/{id}`) and the method in `http.request.method` (falling back to the legacy `http.method`). `handleSpan` matches that `(method, template)` against the statically-extracted `RouteNode` for the service — by `normalizePathTemplate`, so param-syntax differences across frameworks (`{id}` vs `:id` vs `<int:id>`) don't block the match — and mints an OBSERVED `CONTAINS` edge from the serving service to that route, the same structural verb and observed-serving shape GraphQL operations (ADR-122) and gRPC methods (ADR-123) use. This fuses a declared route with its observed traffic, so `get_divergences` compares declared against served at route grain. A route NEAT never extracted has no declared twin to land on, so it mints nothing here — the served-but-undeclared route stays a follow-on, and no observed-only RouteNode is fabricated. Only the serving side — `SERVER` / `INTERNAL` / unkinded spans — is admitted, matching the GraphQL / gRPC / WebSocket serving gates above: a CLIENT span carries `url` / `http.target`, not the served `http.route`, and mints nothing. `INTERNAL` is admitted because some router instrumentation carries the matched `http.route` on an internal request-context span rather than the transport `SERVER` span — NestJS 11's `@opentelemetry/instrumentation-nestjs-core` is the case in point (ADR-155). The `http.route` presence plus the extracted-`RouteNode` match are what keep a generic `INTERNAL` span (business logic, a manual span) from minting anything: no `http.route`, or no declared route to land on, and the gate falls through.

**Next.js API routes name their template in `next.span_name`, not `http.route` (ADR-204).** A Next.js API-route serving span carries the matched template in `next.span_name` — mirrored on the span name — as `executing api route (pages) /api/products/[productId]` (Pages Router) or `executing api route (app) /api/products/[productId]` (App Router), while the same request's `http.target` carries only the concrete path (`/api/products/L9ECAV7KIM`). So `http.route` alone leaves every Next route cold. `nextApiRouteTemplate` reads that phrase and returns the templated route, and the fusion prefers it over `http.route` (`nextApiRouteTemplate(span) ?? span.httpRoute`). The Next bracket syntax in that template — dynamic `[id]`, catch-all `[...slug]`, optional-catch-all `[[...slug]]` — collapses under `normalizePathTemplate` to the same `:param` key the declared `RouteNode` reduces to, so the two fuse onto one node. This is additive: a span without the phrase falls back to `http.route` and every other framework fuses unchanged, and a Next template NEAT never extracted still lands on no node — the served-but-undeclared floor above holds.
...
What's provider-specific — fetching the signal, resolving a `(target, callCount, lastObserved)` tuple to a node id — is out of scope for this contract and lives in `connectors.md` plus each provider's own module. This section exists only to name the seam: connector code calls the same shared mutation primitives `handleSpan` calls, rather than routing observed edges through a second, parallel mechanism. Column grain (ADR-157) rides the same seam: a connector that parses SQL query text to a table (Neon / Supabase `pg_stat_statements`) carries the columns it recovered — via the same shared `columnsFromSqlStatement`, never a second parser — on its `ObservedSignal`, and the pipeline merges them onto the resolved table node through the same `mergeObservedColumns` primitive OTel ingest uses.
...
## What counts as an OBSERVED error on an edge (`errorCount`) (amended — refs #1065)

`errorCount` on the `signal` block is how many of a span-derived edge's observations failed, and it is the signal `get_root_cause` walks its failing-CALLS chain on ([get-root-cause.md](./get-root-cause.md); ADR-209 gates each hop on `errorCount > 0`). A span records an error — `isError`, incrementing `errorCount` by one at `upsertObservedEdge` — when **any** of:

- **`statusCode === 2`** — the span's own OTel status is ERROR. The explicit marker.
- **`rpc.grpc.status_code` present and ≠ 0** — a non-OK gRPC status (0 is OK). OTel's gRPC instrumentation routinely leaves the span *status* UNSET and carries the outcome here.
- **`http.response.status_code` ≥ 500** — an HTTP 5xx server response, which OTel HTTP-client semconv also leaves on an UNSET span.

A status-only gate was blind to the two attribute forms — the dominant failure representation on gRPC/HTTP microservice stacks, where instrumentation routinely leaves span status UNSET and puts the outcome in `rpc.grpc.status_code` (≠ 0) or `http.response.status_code` (≥ 500). Under a status-only gate every such edge reads `errorCount: 0` and `get_root_cause` finds no failing chain to walk. Reading the attributes closes that (`spanRecordsError`, ADR-210).

**HTTP 4xx is excluded from `errorCount`.** A 4xx is a client error, not the callee failing, and a run of 4xx coalesces into its own incident (§What records an incident) rather than counting as an edge error. gRPC has no numeric 4xx/5xx split — any non-zero code is treated as a failure uniformly, matching how `nonHttpFailureMessage` already renders it. An `exception` event feeds the incident ledger (§What records an incident), not `errorCount`; the edge error signal stays the three status/attribute forms above.
...
### The body is decompressed when `Content-Encoding` names gzip or deflate (ADR-211, refs #1068)

The OTLP/HTTP spec permits a gzip-compressed request body, and the standard OpenTelemetry Collector's OTLP exporter **gzip-compresses by default** — so a "bring your own collector" deployment that doesn't decompress silently drops every batch (the JSON parser throws, the protobuf decoder fails, either way a 400). A `preParsing` hook runs ahead of the content-type dispatch above: when the request carries `Content-Encoding: gzip` (or `x-gzip`, or `deflate`), the body streams through Node's `zlib` (`createGunzip` / `createInflate`) so the JSON and protobuf branches both receive plaintext and stay oblivious to compression. No `Content-Encoding` (or `identity`, or an encoding NEAT doesn't decode such as `br`) is a pure pass-through — the uncompressed path is byte-for-byte unchanged, and an unknown encoding fails exactly as before. It **streams** rather than buffer-then-inflates, so the `bodyLimit` above is enforced on the *decompressed* bytes (a decompression bomb is capped at the same 16 MB ceiling), and the transform reports its wire byte count via `receivedEncodedLength` so Fastify's content-length check passes. A truncated or garbage compressed body makes the `zlib` stream error, which Fastify turns into a clean **400** — the same shape a bad protobuf body gets, before any mutation, honoring the non-blocking-receiver discipline (§Non-blocking ingest); the daemon stays up and the next good batch still ingests. Both the bare `/v1/traces` and the project-scoped `/projects/:project/v1/traces` routes go through the one hook. This is confined to request decoding — `parseOtlpRequest`, `handleSpan`, and the symbol-fusion path (`landObservedSymbol`) are untouched. gRPC needs nothing: `@grpc/grpc-js` decompresses at the transport layer before `parseOtlpRequest` ever sees the message.
...
1. **Any failure span → one incident.** A span with `status: ERROR` (`statusCode === 2`), an `events[]` entry named `exception`, `http.response.status_code >= 500`, or a non-OK `rpc.grpc.status_code` (≠ 0) records an incident on its own — the failure signal is what counts, not the presence of HTTP context. A span carrying an ERROR status flows through the §Error-events path above. An `exception` event records an incident even when the span left its status UNSET and carries no HTTP response — the async / queue / background-worker case, a bullmq or Redis-Streams job that throws: `handleSpan` records it, attributed to the span's service and to the handler `file:line` when the span carries `code.filepath`. Its message follows the exception → HTTP context → non-HTTP → `'unknown error'` chain (§Exception data), the same chain every incident write shares. A 5xx records here in `handleSpan` even when its status stays UNSET — OTel HTTP-client semconv leaves a CLIENT span's status UNSET on a 5xx, so a status-only gate is blind to a server-error response; the response-code read closes that. A 5xx that *also* carries ERROR status records once: the response-code path skips `statusCode === 2` so the §Error-events write owns it — and that write attaches the response status onto the incident (reading both the modern `http.response.status_code` and the old-semconv `http.status_code`, coercing the string form an ingress like Envoy sends a 504 as), so `incidentKindOf` classifies it a `5xx` rather than a generic exception. It does not tag `errorType`, which would defeat the synthesized-HTTP-echo trace-dedupe that keys on an absent `errorType` (refs #1118). A **non-OK gRPC status** records the same way (`recordGrpcFailureIncident`, ADR-210, refs #1065): OTel's gRPC instrumentation leaves the span status UNSET and puts the outcome in `rpc.grpc.status_code`, so the status-only and HTTP response-code gates both miss it — the dominant failure form on a gRPC stack. It records immediately like a 5xx (gRPC has no numeric 4xx/5xx split to coalesce on) and never reaches the 4xx-burst path, which is HTTP-only; the branches are mutually exclusive and record at most one incident per span, so nothing double-counts. Its `errorType` is `grpc-failure`, its message names the canonical gRPC status (§Exception data), and the raw `rpc.grpc.status_code` rides along in the passed-through attributes. The HTTP-status path is a subset of failure-span recording; the exact-`(traceId, spanId)` and one-incident-per-request collapses apply unchanged ([ADR-117](../decisions.md#adr-117--incident-recording-covers-any-failure-span-not-only-http-status-amends-adr-033--adr-113) amends ADR-033 / ADR-113).
...
A failing-response incident is attributed to the **source** — the caller whose work is failing is the node a debugging session asks about, so the incident answers "this service's calls to X are failing," not "X failed," and the peer is carried in the message and the passed-through attributes. The source resolves one grain finer when the failing span supports it (`incidentAffectedNode`, ADR-191): a call site (`code.filepath` / `code.lineno` / `code.function`) whose line falls inside a statically-extracted symbol's span attributes to that `symbol:` node — the same span-containment `landObservedSymbol` lands edges by — degrading to the `file:` node, then `serviceId(span.service)`, honestly.

**Stacktrace code-locus recovery when the span carries no `code.*` (ADR-216).** A large class of exception span carries `exception.stacktrace` but no `code.*` attributes — the runtime's own exception recording (`record_exception`) sets none, the dominant shape of Python auto-instrumented gRPC/Flask exceptions. For that class `incidentAffectedNode` parses the stacktrace's **deepest application frame** (the frame nearest the throw site that is not under a runtime/vendor prefix) and runs its path through the same runtime-path → graph-node join a `code.*` call site uses (`relPathForRuntimeFile` → `reconcileObservedRelPath` → `landObservedSymbol`), then synthesizes `code.filepath` / `code.lineno` onto the incident record so `root-cause`, `incidents`, and `get_divergences` (ADR-215's `symbolLocus`) all read the declaring file:line. The frame parser (`stacktrace.ts`) keys strictly on **generic frame syntax and generic vendor-prefix markers** (`File "…", line N, in fn`; `at fn (path:line:col)`; `at pkg.Class.method(File.ext:line)`; markers `node_modules` / `site-packages` / `dist-packages` / the `node:` scheme), a table extended by adding a row — never a language / framework / provider name (ADR-158 §6). The recovery is held to a stricter honesty bar than a stamped call site: it attributes to a code node **only when the recovered frame resolves to a FileNode the graph already carries**, keeping the service attribution otherwise — a stacktrace frame is inferred where a `code.*` attribute is authoritative, so a frame joining to no node fabricates nothing (§6).

The resolution is **read-only** (it mints no observed-only symbol from the incident path, so it holds the non-blocking-receiver discipline), and both incident-write paths use it — the durable receiver write and the daemon-less inline write, which previously reused the outbound edge target. The finer node stays discoverable from its owning file and service (`incidentMatchesNode` is grain-aware). This sits alongside the OBSERVED edge's resolved target; incidents and edges are separate ledgers. `GET /incidents/:nodeId` (which `get_incident_history` wraps) surfaces these on the source node.
