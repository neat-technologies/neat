...
A Next.js Server Action nests under the file the same way (ADR-168). A `ServerActionNode` — an exported `"use server"` function, identified by `serverActionId(service, module, exportName)` → `action:<service>:<module>#<exportName>` — is owned by its declaring file through `file ──CONTAINS──▶ action`, the same containment level as `file ──CONTAINS──▶ symbol`. A client file that references the imported action lands a `file ──CALLS──▶ action` edge, so the client→action call path is a first-class relationship the file originates like any other. This is the same additive shape: an action is a node under the file, and the file stays primary.
