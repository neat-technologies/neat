...
Provenance is not only an edge property. A column NEAT knows about is a provenanced **attribute** on the table node it belongs to — `columns: { name, provenance, confidence }[]` on the `sql-table` / `supabase-table` InfraNode (ADR-157 §1, [schema.md](./schema.md)) — never its own node. The enum is the same four settled values read one grain finer: a column is `OBSERVED` when a production `db.statement` touches it, `EXTRACTED` when a schema/ORM declares it (Phase 2), and `PROV_RANK` decides which wins when both apply — OBSERVED outranks EXTRACTED, so a declared column that production also touches is recorded `OBSERVED` (the observation is the stronger claim). `confidence` is graded in `[0, 1]` the same tiers ADR-066 locks; a column recovered from a real statement is a direct, unambiguous observation (the name is literally in the text, and the parser degrades rather than guess), so it lands high but not at the `1.0` an edge earns only from strong recent traffic — per-column volume grading is a future refinement. The merge lives in `ingest.ts` (`mergeObservedColumns`) per the lifecycle authority (ADR-030), and never duplicates a name.
