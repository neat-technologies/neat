...
A new `InfraNode` **kind** is not a schema change at all — `kind` is `z.string().optional()`, an open sub-type per ADR-022 (`kafka-topic`, `sql-table`, `cloudflare-kv`, …), not an enum. [ADR-170](../decisions.md#adr-170) adds a `zod-schema` kind (a top-level `z.object` / `z.enum` literal minted at `infraId('zod-schema', <name>)`, its declared fields folded on as the same `ColumnAttr` grain), and because `kind` is a free string this is neither growth nor shape: no `NodeType`, no `SCHEMA_VERSION` step, no snapshot diff. The node reuses the existing `InfraNodeSchema` — a new value in an open string field the schema already accepts.
