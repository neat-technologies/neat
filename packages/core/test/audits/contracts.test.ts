...
describe('extend-skill contract', () => {
  const OTEL_INIT_CONTENT = [
    '// Generated by `neat init --apply` (ADR-069). OpenTelemetry auto-instrumentation hook.',
  ...
  const PRISMA_PKG = JSON.stringify({
    name: 'prisma-app',
    version: '1.0.0',
    dependencies: { '@prisma/client': '^6.1.0', express: '^4.18.0' },
  })
  ...
  async function makeFixture(files: Record<string, string>): Promise<string> {
    ...
    const os2 = await import('node:os')
    const path2 = await import('node:path')
    const dir = await fs2.mkdtemp(path2.join(os2.tmpdir(), 'extend-contract-'))
    for (const [rel, content] of Object.entries(files)) {
  ...
  it('listUninstrumented filters to first-party/third-party/gap only (no bundled, no http-only)', async () => {
    const { listUninstrumented } = await import('../../src/extend/index.js')
    ...
    const results = await listUninstrumented({ project: 'test', scanPath: dir })
    const names = results.map((r) => r.library)
    const coverages = results.map((r) => r.coverage)
    // bundled (express) and http-only (stripe) must not appear
    expect(names).not.toContain('express')
    expect(names).not.toContain('stripe')
    // first-party (@prisma/client) and third-party (openai) must appear
    expect(names).toContain('@prisma/client')
    expect(names).toContain('openai')
    // every returned entry is first-party, third-party, or gap
    for (const c of coverages) {
      expect(['first-party', 'third-party', 'gap']).toContain(c)
    }
  })

  // Stub that avoids hitting the real npm registry in tests.
  const stubRunInstall = async (cmd: { pm: string; cwd: string; args: string[] }) => ({
    pm: cmd.pm as 'npm',
    cwd: cmd.cwd,
    args: cmd.args,
    exitCode: 0 as const,
  ...
  it('applyExtension is idempotent — second call returns alreadyApplied: true', async () => {
    const { applyExtension } = await import('../../src/extend/index.js')
    const os2 = await import('node:os')
    const path2 = await import('node:path')
    const logPath = path2.join(os2.tmpdir(), `extend-log-${Date.now()}.ndjson`)
    process.env.NEAT_EXTEND_LOG = logPath
    const dir = await makeFixture({ 'package.json': PRISMA_PKG, 'otel-init.js': OTEL_INIT_CONTENT })
    const args = {
      library: '@prisma/client',
      instrumentation_package: '@prisma/instrumentation',
      version: '^6.0.0',
      registration_snippet: "instrumentations.push(new (require('@prisma/instrumentation').PrismaInstrumentation)())",
    }
    try {
      const first = await applyExtension({ project: 'test', scanPath: dir }, args, { runInstall: stubRunInstall })
      expect(first.alreadyApplied).toBe(false)
      const second = await applyExtension({ project: 'test', scanPath: dir }, args, { runInstall: stubRunInstall })
  ...
  it('applyExtension only writes to instrumentation/otel-init files and package.json', async () => {
    const { applyExtension } = await import('../../src/extend/index.js')
    const { promises: fs2 } = await import('node:fs')
    const os2 = await import('node:os')
    const path2 = await import('node:path')
    ...
    const dir = await makeFixture({
      ...
      'otel-init.js': OTEL_INIT_CONTENT,
    ...
    try {
      const result = await applyExtension({ project: 'test', scanPath: dir }, args, { runInstall: stubRunInstall })
      for (const f of result.filesTouched) {
        const base = path2.basename(f)
        const allowed =
          base === 'package.json' ||
          base.startsWith('otel-init') ||
          base.startsWith('instrumentation')
        expect(allowed, `${f} is outside the allowed file scope`).toBe(true)
      }
      const srcContent = await fs2.readFile(path2.join(dir, 'src/index.js'), 'utf8')
      expect(srcContent).toBe('require("./otel-init")')
    } finally {
      delete process.env.NEAT_EXTEND_LOG
    }
  })

  it('rollbackExtension undoes the last apply', async () => {
    const { applyExtension, rollbackExtension } = await import('../../src/extend/index.js')
    const { promises: fs2 } = await import('node:fs')
    const os2 = await import('node:os')
    const path2 = await import('node:path')
    const logPath = path2.join(os2.tmpdir(), `extend-log-${Date.now()}.ndjson`)
    ...
    try {
      await applyExtension({ project: 'test', scanPath: dir }, args, { runInstall: stubRunInstall })
      const pkgAfter = JSON.parse(await fs2.readFile(path2.join(dir, 'package.json'), 'utf8'))
      expect(pkgAfter.dependencies['@prisma/instrumentation']).toBe('^6.0.0')
      const rollback = await rollbackExtension({ project: 'test', scanPath: dir }, { library: '@prisma/client' })
      expect(rollback.undone).toBe(true)
      const pkgRolled = JSON.parse(await fs2.readFile(path2.join(dir, 'package.json'), 'utf8'))
      expect(pkgRolled.dependencies['@prisma/instrumentation']).toBeUndefined()
      const hookContent = await fs2.readFile(path2.join(dir, 'otel-init.js'), 'utf8')
      expect(hookContent).not.toContain(args.registration_snippet)
    } finally {
      delete process.env.NEAT_EXTEND_LOG
    }
  })

  it('dryRunExtension makes no writes', async () => {
    const { dryRunExtension } = await import('../../src/extend/index.js')
    const { promises: fs2 } = await import('node:fs')
    const os2 = await import('node:os')
    const path2 = await import('node:path')
    const logPath = path2.join(os2.tmpdir(), `extend-log-${Date.now()}.ndjson`)
    process.env.NEAT_EXTEND_LOG = logPath
    const dir = await makeFixture({ 'package.json': PRISMA_PKG, 'otel-init.js': OTEL_INIT_CONTENT })
    const pkgBefore = await fs2.readFile(path2.join(dir, 'package.json'), 'utf8')
    const hookBefore = await fs2.readFile(path2.join(dir, 'otel-init.js'), 'utf8')
    try {
      const diff = await dryRunExtension(
      ...
      expect(await fs2.readFile(path2.join(dir, 'package.json'), 'utf8')).toBe(pkgBefore)
      expect(await fs2.readFile(path2.join(dir, 'otel-init.js'), 'utf8')).toBe(hookBefore)
      expect(existsSync(logPath)).toBe(false)
      expect(diff.depsToAdd).toContain('@prisma/instrumentation@^6.0.0')
      expect(diff.filesTouched.length).toBeGreaterThan(0)
    } finally {
      delete process.env.NEAT_EXTEND_LOG
    }
  })
