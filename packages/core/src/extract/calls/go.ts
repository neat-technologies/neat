...
// Go raw-SQL data-axis recognizer. ADR-154 seeded this with the six `database/sql`
// query/exec methods and the table alone; ADR-184 rounds it out into a real data
// reader — the sqlx surface, the touched columns, and a structural import gate.
//
...
//
// GORM (ADR-180) covers the ORM case; plenty of Go services skip the ORM and hand
// `database/sql` or `github.com/jmoiron/sqlx` a literal SQL string, and those table
// reads/writes land on the SAME `infra:sql-table:<name>` nodes every ORM reader
// produces.
//
// THE POINT is fusion, not a static table list (that's what every other tool already
// does). The OBSERVED ingest recovers the table by running `tableFromSqlStatement` on
// the span's `db.statement` / `db.query.text` — there is no dedicated table attribute
// (ADR-152) — so this recognizer runs the EXACT SAME helper on the SAME SQL text. The
// extracted table name is therefore byte-identical to the observed one and the two
// fuse on `infraId('sql-table', <name>)` rather than twinning. `columnsFromSqlStatement`
// does the same for the touched columns, folded as `ColumnAttr` (ADR-157 §3). Don't
// write a second SQL parser here — divergence from the ingest parse is the one way
// this breaks.
//
// Import gate (mirrors net/http's structural gate in routes.ts): recognize only in a
// file that imports `database/sql` and/or `github.com/jmoiron/sqlx`. Without it an
// arbitrary `.Query(...)` / `.Exec(...)` on any object — an HTTP client, a template
// engine — mints a phantom table from any string that happens to parse as SQL. The
// sqlx-only methods (`Get`, `Select`, …) are common names, so they are recognized
// only when sqlx is actually imported.
...
// the SQL isn't argument 0; the "first string-literal argument" scan below handles
...
// Whether the file imports any of `names` — the structural half of the gate, read off
// both the grouped `import ( … )` and the single `import "database/sql"` forms (each
// yields an `import_spec` whose path is a string literal). Mirrors `goImportsNetHttp`
// in routes.ts.
...
// contribute their columns — the orchestrator (calls/index.ts) dedupes the CALLS edge
// and folds the union of columns onto the one node.
