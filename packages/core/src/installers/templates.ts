...
export const OTEL_INIT_STAMP =
  '// neat-template-version: 8 — CommonJS TypeScript uses a synchronous bootstrap (#910); OTel init degrades to no-OBSERVED when dependencies are absent (#820); daemon.json endpoint resolution (ADR-096) + layered file-first capture (ADR-090).'
...
// TypeScript compiled as CommonJS cannot contain top-level await. Keep the
// generated file as .ts so tsc copies it beside the compiled entry, while the
// synchronous require bootstrap preserves initialization ordering.
export const OTEL_INIT_TS_CJS = OTEL_INIT_CJS.replace(
  `${OTEL_INIT_HEADER}\n${OTEL_INIT_STAMP}`,
