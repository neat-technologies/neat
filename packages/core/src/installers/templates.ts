...
// .env.neat shape (ADR-069 §4, amended v0.4.1 — refs #339). Two keys:
// OTEL_SERVICE_NAME (the registered project name, shared across every package
// the orchestrator installs into) and OTEL_EXPORTER_OTLP_ENDPOINT (matches
// the daemon's OTLP bind at :4318). The project name is the routing key the
// daemon owns; carrying it on every package means the OTLP wire and the
// registry agree on the same identity end-to-end. The parameter is just a
// string so ad-hoc test fixtures can pass a package name and still get a
// well-formed `.env.neat` without threading the orchestrator's project-
// resolution chain.
export function renderEnvNeat(projectName: string): string {
  return [
