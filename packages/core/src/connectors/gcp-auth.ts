// GCP service-account token source — the refreshable credential for the GCP connectors
// (docs/contracts/connector-config.md §9, ADR-230).
//
...
// calls the returned source each tick (the shipped `refreshCredentials` seam,
// index.ts); the connector still receives an already-minted `{ projectId,
// accessToken }` and never sees the refresh.
//
// The JWT-bearer flow follows Google's own docs
...
/** The narrow Cloud Logging read scope the GCP connectors need (cloud-run, gcp-lb, firebase). */
export const GCP_LOGGING_READ_SCOPE = 'https://www.googleapis.com/auth/logging.read'
 ...
 * returned source mints on first call and re-mints only when the cached token
 * is within `refreshSkewMs` of expiry — so the poll loop can call it every tick
 * cheaply. It hands back `{ projectId, accessToken }`, the exact record shape
 * the GCP connectors already read, so no connector changes.
 *
